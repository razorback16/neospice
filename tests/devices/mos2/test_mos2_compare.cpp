...
class Mos2Validation : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(Mos2Validation, NmosOperatingPoint) {
    std::string cir_path = std::string(TEST_CIRCUITS_DIR) + "/mos2_nmos_dc_op.cir";
...
TEST_F(Mos2Validation, PmosOperatingPoint) {
    std::string cir_path = std::string(TEST_CIRCUITS_DIR) + "/mos2_pmos_dc_op.cir";
    ...
    // Verify PMOS physics
    double v_drain = cs_result.voltage("drain");
    EXPECT_GT(v_drain, 0.01) << "PMOS drain should be pulled up";
...
TEST_F(Mos2Validation, NmosAcResponse) {
    std::string cir_path = std::string(TEST_CIRCUITS_DIR) + "/mos2_nmos_ac.cir";
