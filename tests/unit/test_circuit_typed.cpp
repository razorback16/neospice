...
TEST(CircuitTyped, ResistorDivider) {
    ...
    ckt.V("V1", in, GROUND_INTERNAL, 10.0);
    ckt.R("R1", in, out, 1e3);
    ckt.R("R2", out, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto result = solve_dc(ckt);
    EXPECT_NEAR(result.voltage(NodeId{out}), 5.0, 1e-6);
}
...
TEST(CircuitTyped, RCTransient) {
    ...
    ckt.R("R1", in, out, 1e3);
    ckt.C("C1", out, GROUND_INTERNAL, 1e-6);
    ckt.finalize();
    auto result = solve_transient(ckt, 1e-6, 5e-3);
    auto v = result.voltage(NodeId{out});
...
TEST(CircuitTyped, VSourceWithAC) {
    ...
    auto in = ckt.node("in");
    auto out = ckt.node("out");
    ckt.V("V1", in, GROUND_INTERNAL, 5.0, 1.0);  // dc=5, ac=1
    ckt.R("R1", in, out, 1e3);
    ckt.C("C1", out, GROUND_INTERNAL, 1e-9);
    ckt.finalize();
    auto dc = solve_dc(ckt);
    EXPECT_NEAR(dc.voltage(NodeId{in}), 5.0, 1e-6);
...
TEST(CircuitTyped, CurrentSource) {
    Circuit ckt;
    auto n1 = ckt.node("n1");
    ckt.I("I1", GROUND_INTERNAL, n1, 1e-3);
    ckt.R("R1", n1, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto result = solve_dc(ckt);
    EXPECT_NEAR(result.voltage(NodeId{n1}), 1.0, 1e-6);
}
...
TEST(CircuitTyped, VCVS) {
    ...
    auto out = ckt.node("out");
    ckt.V("V1", ctrl, GROUND_INTERNAL, 2.0);
    ckt.R("R1", ctrl, GROUND_INTERNAL, 1e3);
    ckt.E("E1", out, GROUND_INTERNAL, ctrl, GROUND_INTERNAL, 5.0);
    ckt.R("R2", out, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto dc = solve_dc(ckt);
    EXPECT_NEAR(dc.voltage(NodeId{out}), 10.0, 1e-6);
}
...
TEST(CircuitTyped, VCCS) {
    ...
    auto in = ckt.node("in");
    auto out = ckt.node("out");
    ckt.V("V1", in, GROUND_INTERNAL, 1.0);
    ...
    ckt.G("G1", out, GROUND_INTERNAL, in, GROUND_INTERNAL, 0.001);
    ckt.R("R2", out, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto dc = solve_dc(ckt);
    ...
    EXPECT_NEAR(dc.voltage(NodeId{out}), -1.0, 1e-6);
}
...
TEST(CircuitTyped, CCCS) {
    ...
    auto in = ckt.node("in");
    auto out = ckt.node("out");
    DevId v1 = ckt.V("V1", in, GROUND_INTERNAL, 5.0);
    ckt.R("R1", in, GROUND_INTERNAL, 1e3);
    ckt.F("F1", out, GROUND_INTERNAL, v1, 2.0);
    ckt.R("R2", out, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto dc = solve_dc(ckt);
...
TEST(CircuitTyped, CCVS) {
    ...
    DevId v1 = ckt.V("V1", in, GROUND_INTERNAL, 5.0);
    ckt.R("R1", in, GROUND_INTERNAL, 1e3);
    ckt.H("H1", out, GROUND_INTERNAL, v1, 1000.0);
    ckt.R("R2", out, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto dc = solve_dc(ckt);
    EXPECT_TRUE(dc.status.converged);
...
TEST(CircuitTyped, Inductor) {
    ...
    DevId l = ckt.L("L1", out, GROUND_INTERNAL, 1e-3);
...
TEST(CircuitTyped, AddDevReturnsDevId) {
    Circuit ckt;
    auto n = ckt.node("n1");
    DevId id = ckt.add_dev(std::make_unique<Resistor>("R1", n, GROUND_INTERNAL, 1e3));
...
TEST(CircuitTyped, CoupledInductors) {
    ...
    DevId l1 = ckt.L("L1", mid, GROUND_INTERNAL, 1e-3);
    DevId l2 = ckt.L("L2", out, GROUND_INTERNAL, 1e-3);
    ckt.K("K1", l1, l2, 0.5);
    ckt.R("R2", out, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto dc = solve_dc(ckt);
...
TEST(CircuitTyped, ISourceWithAC) {
    Circuit ckt;
    auto n1 = ckt.node("n1");
    ckt.I("I1", GROUND_INTERNAL, n1, 1e-3, 1e-3);  // dc=1mA, ac=1mA
    ckt.R("R1", n1, GROUND_INTERNAL, 1e3);
    ckt.finalize();
    auto dc = solve_dc(ckt);
    EXPECT_NEAR(dc.voltage(NodeId{n1}), 1.0, 1e-6);
}
