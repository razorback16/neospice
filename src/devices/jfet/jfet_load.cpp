...
#endif
#ifndef IOPR
#define IOPR(a,b,c,d) {a, b, (Shim::IF_SET|Shim::IF_ASK|c), d}
#endif
#ifndef IOPA
...
int
JFETload(JFETModel *inModel, Shim::Ckt *ckt)
            ...
            if( (!(ckt->CKTmode & MODEINITFIX)) |
                ...
                if((icheck == 1) ||
                   ...
                   (fabs(cdhat-cd) > ckt->CKTreltol *
                    ...
                    ckt->CKTnoncon++;
                    ckt->CKTtroubleElt = (JFETInstance *) here;
                }
            }
...
load:
            ...
            *(ckt->CKTrhs + here->JFETsourcePrimeNode) +=
             ...
             *    load y matrix 
             */
            mat.add(here->JFETdrainDrainPrimePtr, m * (-gdpr));
            mat.add(here->JFETgateDrainPrimePtr, m * (-ggd));
            mat.add(here->JFETgateSourcePrimePtr, m * (-ggs));
            mat.add(here->JFETsourceSourcePrimePtr, m * (-gspr));
            mat.add(here->JFETdrainPrimeDrainPtr, m * (-gdpr));
            mat.add(here->JFETdrainPrimeGatePtr, m * (gm-ggd));
            mat.add(here->JFETdrainPrimeSourcePrimePtr, m * (-gds-gm));
            mat.add(here->JFETsourcePrimeGatePtr, m * (-ggs-gm));
            mat.add(here->JFETsourcePrimeSourcePtr, m * (-gspr));
            mat.add(here->JFETsourcePrimeDrainPrimePtr, m * (-gds));
            mat.add(here->JFETdrainDrainPtr, m * (gdpr));
            mat.add(here->JFETgateGatePtr, m * (ggd+ggs));
            mat.add(here->JFETsourceSourcePtr, m * (gspr));
            mat.add(here->JFETdrainPrimeDrainPrimePtr, m * (gdpr+gds+ggd));
            mat.add(here->JFETsourcePrimeSourcePrimePtr, m * (gspr+gds+gm+ggs));
        }
    }
    return 0;
}
