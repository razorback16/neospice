...
int CKTmkCur(Ckt *ckt, CKTnode **node_out,
             const char *basename, const char *suffix) {
    return CKTmkVolt(ckt, node_out, basename, suffix);
}
...
// Logarithmic damping of the per-iteration change of deltemp beyond LIM_TOL.
// Faithful port of ngspice devsup.c:DEVlimitlog.
double DEVlimitlog(double deltemp, double deltemp_old, double LIM_TOL, int *check) {
    *check = 0;
    if (std::isnan(deltemp) || std::isnan(deltemp_old)) {
        deltemp = 0.0;
        *check = 1;
        return deltemp;
    }
    if (deltemp > deltemp_old + LIM_TOL) {
        deltemp = deltemp_old + LIM_TOL + std::log10((deltemp - deltemp_old) / LIM_TOL);
        *check = 1;
    } else if (deltemp < deltemp_old - LIM_TOL) {
        deltemp = deltemp_old - LIM_TOL - std::log10((deltemp_old - deltemp) / LIM_TOL);
        *check = 1;
    }
    return deltemp;
}

// Predictor extrapolation for the next time point (ngspice devsup.c:DEVpred).
...
// Non-linear gate-drain / gate-source capacitance (ngspice devsup.c:DevCapVDMOS).
void DevCapVDMOS(double vgd, double cgdmin, double cgdmax, double a, double cgs,
                 double *capgs, double *capgd) {
    double s = (cgdmax - cgdmin) / (1 + M_PI / 2);
    double y = cgdmax - s;
    if (vgd > 0)
        *capgd = 0.5 * (s * std::tanh(a * vgd) + y);
    else
        *capgd = 0.5 * (s * std::atan(a * vgd) + y);
    *capgs = 0.5 * cgs;
}
