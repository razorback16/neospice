...
int
VDMOSsetup(Shim::Matrix *matrix, VDMOSModel *inModel, Shim::Ckt *ckt,
    int *states)
{
            ...
            *states += VDMOSnumStates;
            ...
            if (model->VDMOSdrainResistance > 0) {
                if (here->VDMOSdNodePrime == 0) {
                    error = Shim::CKTmkVolt(ckt, &tmp, here->VDMOSname, "drain");
            ...
            if (model->VDMOSgateResistance > 0 ) {
                if (here->VDMOSgNodePrime == 0) {
                    error = Shim::CKTmkVolt(ckt, &tmp, here->VDMOSname, "gate");
                    if (error) return(error);
            ...
            if (model->VDMOSsourceResistance > 0) {
                if (here->VDMOSsNodePrime == 0) {
                    error = Shim::CKTmkVolt(ckt, &tmp, here->VDMOSname, "source");
                    if (error) return(error);
            ...
            if (model->VDIOresistance > 0 ) {
                if (here->VDIOposPrimeNode == 0) {
                    error = Shim::CKTmkVolt(ckt, &tmp, here->VDMOSname, "body diode");
                    if (error) return(error);
            ...
            if ((here->VDMOSthermal) && (model->VDMOSrthjcGiven)) {
               if (here->VDMOStempNode == -1) {
                  error = Shim::CKTmkVolt(ckt,&tmp,here->VDMOSname,"Tj");
                  if (error) return(error);
                     here->VDMOStempNode = tmp->number;
               }
               if (here->VDMOStcaseNode == -1) {
                  error = Shim::CKTmkVolt(ckt,&tmp,here->VDMOSname,"Tc");
                  if (error) return(error);
               ...
               if(here->VDMOSvcktTbranch == 0) {
                ...
                if (here->VDMOStNodePrime == 0) {
                    error = Shim::CKTmkVolt(ckt, &tmp, here->VDMOSname, "cktTemp");
                    if (error) return(error);
...
int
VDMOSunsetup(VDMOSModel *inModel, Shim::Ckt *ckt)
{
    VDMOSModel *model;
    VDMOSInstance *here;
        ...
        model = VDMOSnextModel(model))
    {
            ...
            here = VDMOSnextInstance(here))
        {
            if (here->VDMOSsNodePrime > 0
                && here->VDMOSsNodePrime != here->VDMOSsNode)
                Shim::CKTdltNNum(ckt, here->VDMOSsNodePrime);
            here->VDMOSsNodePrime = 0;

            if (here->VDMOSdNodePrime > 0
                && here->VDMOSdNodePrime != here->VDMOSdNode)
                Shim::CKTdltNNum(ckt, here->VDMOSdNodePrime);
            here->VDMOSdNodePrime = 0;

            if (here->VDMOSgNodePrime > 0
                && here->VDMOSgNodePrime != here->VDMOSgNode)
                Shim::CKTdltNNum(ckt, here->VDMOSgNodePrime);
            here->VDMOSgNodePrime = 0;

            if (here->VDIOposPrimeNode > 0
                && here->VDIOposPrimeNode != here->VDMOSsNode)
                Shim::CKTdltNNum(ckt, here->VDIOposPrimeNode);
            here->VDIOposPrimeNode = 0;

            if ((here->VDMOSthermal) && (model->VDMOSrthjcGiven)) {
                if (here->VDMOStNodePrime > 0)
                    Shim::CKTdltNNum(ckt, here->VDMOStNodePrime);
                here->VDMOStNodePrime = 0;
                if (here->VDMOSvcktTbranch > 0)
                    Shim::CKTdltNNum(ckt, here->VDMOSvcktTbranch);
