...
void VDMOStempUpdate(VDMOSModel *inModel, VDMOSInstance *here, double Temp, Shim::Ckt *ckt) {
    ...
    * temperature adjusted basic parameters */
    if (model->VDIObvGiven) {
        ...
        if (cbv < here->VDIOtSatCur * tBreakdownVoltage / vt) {
            ...
            Shim::report_error(Shim::ERR_WARNING, "%s: breakdown current increased to %g to resolve", here->VDMOSname, cbv);
            Shim::report_error(Shim::ERR_WARNING,
...
#ifdef TRACE
            Shim::report_error(Shim::ERR_WARNING, "%s: unable to match forward and reverse diode regions: bv = %g, ibv = %g", here->VDMOSname, xbv, xcbv);
#endif
        }
    matched:
...
int
VDMOStemp(VDMOSModel *inModel, Shim::Ckt *ckt)
{
    VDMOSModel *model = inModel;
    VDMOSInstance *here;
