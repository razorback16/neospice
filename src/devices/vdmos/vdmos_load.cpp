...
int
VDMOSload(VDMOSModel *inModel, Shim::Ckt *ckt)
/* actually load the current value into the
 * sparse matrix previously provided
 */
{
    VDMOSModel *model = inModel;
    VDMOSInstance *here;
    neospice::NumericMatrix &mat = *ckt->mat;
    double Beta;
    double arg;
    double cdhat;
    double cdrain;
...
bypass:
            ...
            *(ckt->CKTrhs + here->VDMOSsNodePrime) +=   cdreq + model->VDMOStype * ceqgs;
             ...
             *  load y matrix
             */
            mat.add(here->VDMOSDdPtr, (here->VDMOSdrainConductance + here->VDMOSdsConductance));
            mat.add(here->VDMOSGgPtr, (here->VDMOSgateConductance));
            mat.add(here->VDMOSSsPtr, (here->VDMOSsourceConductance + here->VDMOSdsConductance));
            mat.add(here->VDMOSDPdpPtr, (here->VDMOSdrainConductance + here->VDMOSgds +
                 xrev*(here->VDMOSgm) + gcgd));
            mat.add(here->VDMOSSPspPtr, (here->VDMOSsourceConductance + here->VDMOSgds +
                 xnrm*(here->VDMOSgm) + gcgs));
            mat.add(here->VDMOSGPgpPtr, (here->VDMOSgateConductance) + (gcgd + gcgs));
            mat.add(here->VDMOSGgpPtr, (-here->VDMOSgateConductance));
            mat.add(here->VDMOSDdpPtr, (-here->VDMOSdrainConductance));
            mat.add(here->VDMOSGPgPtr, (-here->VDMOSgateConductance));
            mat.add(here->VDMOSGPdpPtr, -(gcgd));
            mat.add(here->VDMOSGPspPtr, -(gcgs));
            mat.add(here->VDMOSSspPtr, (-here->VDMOSsourceConductance));
            mat.add(here->VDMOSDPdPtr, (-here->VDMOSdrainConductance));
            mat.add(here->VDMOSDPgpPtr, ((xnrm - xrev)*here->VDMOSgm - gcgd));
            mat.add(here->VDMOSDPspPtr, (-here->VDMOSgds - xnrm*
                                      (here->VDMOSgm)));
            mat.add(here->VDMOSSPgpPtr, (-(xnrm - xrev)*here->VDMOSgm - gcgs));
            mat.add(here->VDMOSSPsPtr, (-here->VDMOSsourceConductance));
            mat.add(here->VDMOSSPdpPtr, (-here->VDMOSgds - xrev*
                                      (here->VDMOSgm)));

            mat.add(here->VDMOSDsPtr, (-here->VDMOSdsConductance));
            mat.add(here->VDMOSSdPtr, (-here->VDMOSdsConductance));

            if (selfheat)
            {

                mat.add(here->VDMOSDtempPtr, dIrd_dT);
                mat.add(here->VDMOSDPtempPtr, GmT - dIrd_dT);
                mat.add(here->VDMOSSPtempPtr, -GmT);
                mat.add(here->VDMOSGPtempPtr, 0.0);
                mat.add(here->VDMOSTemptempPtr, -gTtt - dIrd_dT*Vrd + 1/model->VDMOSrthjc + gcTt);
                mat.add(here->VDMOSTempgpPtr, -gTtg);
                mat.add(here->VDMOSTempdPtr, -dIth_dVrd);
                mat.add(here->VDMOSTempdpPtr, -gTtdp + dIth_dVrd);
                mat.add(here->VDMOSTempspPtr, -gTtsp);
                mat.add(here->VDMOSTemptcasePtr, -1/model->VDMOSrthjc);
                mat.add(here->VDMOSTcasetempPtr, -1/model->VDMOSrthjc);
                mat.add(here->VDMOSTcasetcasePtr, 1/model->VDMOSrthjc + 1/model->VDMOSrthca);
                mat.add(here->VDMOSTptpPtr, 1/model->VDMOSrthca);
                mat.add(here->VDMOSTptcasePtr, -1/model->VDMOSrthca);
                mat.add(here->VDMOSTcasetpPtr, -1/model->VDMOSrthca);
                mat.add(here->VDMOSCktTtpPtr, 1.0);
                mat.add(here->VDMOSTpcktTPtr, 1.0);
            }
            ...
            */
            ...
            if ((Check_th == 1) || (Check_dio == 1)) {
                ckt->CKTnoncon++;
                ckt->CKTtroubleElt = (VDMOSInstance *) here;
            }
...
load:
            ...
            *   load matrix
            */
            mat.add(here->VDMOSSsPtr, gspr);
            mat.add(here->VDMOSDdPtr, gd);
            mat.add(here->VDIORPrpPtr, (gd + gspr));
            mat.add(here->VDIOSrpPtr, -gspr);
            mat.add(here->VDIODrpPtr, -gd);
            mat.add(here->VDIORPsPtr, -gspr);
            mat.add(here->VDIORPdPtr, -gd);
            if (selfheat) {
                mat.add(here->VDMOStempSPtr, -dIth_dVrs);
                mat.add(here->VDIOTempposPrimePtr, -dIth_dVdio + dIth_dVrs);
                mat.add(here->VDMOSTempdPtr, dIth_dVdio);
                mat.add(here->VDMOSTemptempPtr, -dIth_dT);
                mat.add(here->VDIOPosPrimetempPtr, dIdio_dT - dIrs_dT);
                mat.add(here->VDMOSSTempPtr, dIrs_dT);
                mat.add(here->VDMOSDtempPtr, -dIdio_dT);
            }
        }
    }
    return 0;
