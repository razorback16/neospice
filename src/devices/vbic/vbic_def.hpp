...
struct VBICInstance {
    VBICModel *VBICmodPtr;    /* backpointer to model */
    VBICInstance *VBICnextInstance;   /* pointer to next instance of
                                                 current model*/
    const char *VBICname;  /* pointer to character string naming this instance */
    ...
    neospice::MatrixOffset VBICcollCollPtr{-1};  /* pointer to sparse matrix at
                             * (collector,collector) */
    neospice::MatrixOffset VBICbaseBasePtr{-1}; /* pointer to sparse matrix at
                             * (base,base) */
    neospice::MatrixOffset VBICemitEmitPtr{-1}; /* pointer to sparse matrix at
                             * (emitter,emitter) */
    neospice::MatrixOffset VBICsubsSubsPtr{-1};   /* pointer to sparse matrix at
                             * (substrate,substrate) */
    neospice::MatrixOffset VBICcollCXCollCXPtr{-1}; /* pointer to sparse matrix at
                             * (collector prime,collector prime) */
    neospice::MatrixOffset VBICcollCICollCIPtr{-1}; /* pointer to sparse matrix at
                             * (collector prime,collector prime) */
    neospice::MatrixOffset VBICbaseBXBaseBXPtr{-1};   /* pointer to sparse matrix at
                             * (base prime,base prime) */
    neospice::MatrixOffset VBICbaseBIBaseBIPtr{-1}; /* pointer to sparse matrix at
                             * (collector prime,collector prime) */
    neospice::MatrixOffset VBICbaseBPBaseBPPtr{-1}; /* pointer to sparse matrix at
                             * (collector prime,collector prime) */
    neospice::MatrixOffset VBICemitEIEmitEIPtr{-1};   /* pointer to sparse matrix at
                             * (emitter prime,emitter prime) */
    neospice::MatrixOffset VBICsubsSISubsSIPtr{-1};    /* pointer to sparse matrix at
                             * (substrate prime, substrate prime) */

    neospice::MatrixOffset VBICbaseEmitPtr{-1}; /* pointer to sparse matrix at
                             * (base,emit) */
    neospice::MatrixOffset VBICemitBasePtr{-1}; /* pointer to sparse matrix at
                             * (emit,base) */
    neospice::MatrixOffset VBICbaseCollPtr{-1}; /* pointer to sparse matrix at
                             * (base,coll) */
    neospice::MatrixOffset VBICcollBasePtr{-1}; /* pointer to sparse matrix at
                             * (coll,base) */
    neospice::MatrixOffset VBICcollCollCXPtr{-1};    /* pointer to sparse matrix at
                             * (collector,collector prime) */
    neospice::MatrixOffset VBICbaseBaseBXPtr{-1};    /* pointer to sparse matrix at
                             * (base,base prime) */
    neospice::MatrixOffset VBICemitEmitEIPtr{-1};    /* pointer to sparse matrix at
                             * (emitter,emitter prime) */
    neospice::MatrixOffset VBICsubsSubsSIPtr{-1};    /* pointer to sparse matrix at
                             * (substrate, Substrate connection) */
    neospice::MatrixOffset VBICcollCXCollCIPtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,base prime) */
    neospice::MatrixOffset VBICcollCXBaseBXPtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,collector prime) */
    neospice::MatrixOffset VBICcollCXBaseBIPtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,collector prime) */
    neospice::MatrixOffset VBICcollCXBaseBPPtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,base prime) */
    neospice::MatrixOffset VBICcollCIBaseBIPtr{-1}; /* pointer to sparse matrix at
                             * (collector prime,base) */
    neospice::MatrixOffset VBICcollCIEmitEIPtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,emitter prime) */
    neospice::MatrixOffset VBICbaseBXBaseBIPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICbaseBXEmitEIPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICbaseBXBaseBPPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICbaseBXSubsSIPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICbaseBIEmitEIPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICbaseBPSubsSIPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */

    neospice::MatrixOffset VBICcollCXCollPtr{-1};  /* pointer to sparse matrix at
                             * (collector prime,collector) */
    neospice::MatrixOffset VBICbaseBXBasePtr{-1};    /* pointer to sparse matrix at
                             * (base prime,base ) */
    neospice::MatrixOffset VBICemitEIEmitPtr{-1};    /* pointer to sparse matrix at
                             * (emitter prime,emitter) */
    neospice::MatrixOffset VBICsubsSISubsPtr{-1};    /* pointer to sparse matrix at
                             * (Substrate connection, substrate) */
    neospice::MatrixOffset VBICcollCICollCXPtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,base prime) */
    neospice::MatrixOffset VBICbaseBICollCXPtr{-1};    /* pointer to sparse matrix at
                             * (base prime,collector prime) */
    neospice::MatrixOffset VBICbaseBPCollCXPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICbaseBXCollCIPtr{-1}; /* pointer to sparse matrix at
                             * (base,collector prime) */
    neospice::MatrixOffset VBICbaseBICollCIPtr{-1}; /* pointer to sparse matrix at
                             * (base,collector prime) */
    neospice::MatrixOffset VBICemitEICollCIPtr{-1};    /* pointer to sparse matrix at
                             * (emitter prime,collector prime) */
    neospice::MatrixOffset VBICbaseBPCollCIPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICsubsSICollCIPtr{-1};   /* pointer to sparse matrix at
                             * (substrate,collector prime) */
    neospice::MatrixOffset VBICbaseBIBaseBXPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICemitEIBaseBXPtr{-1};   /* pointer to sparse matrix at
                             * (emitter prime,base prime) */
    neospice::MatrixOffset VBICbaseBPBaseBXPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICsubsSIBaseBXPtr{-1};   /* pointer to sparse matrix at
                             * (substrate,substrate) */
    neospice::MatrixOffset VBICemitEIBaseBIPtr{-1};   /* pointer to sparse matrix at
                             * (emitter prime,base prime) */
    neospice::MatrixOffset VBICbaseBPBaseBIPtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset VBICsubsSIBaseBIPtr{-1};   /* pointer to sparse matrix at
                             * (substrate,base prime) */
    neospice::MatrixOffset VBICsubsSIBaseBPPtr{-1};   /* pointer to sparse matrix at
                             * (substrate,substrate) */
...
/* per model data */
struct VBICModel {           /* model structure for a vbic */
    int VBICmodType;                  /* type index of this device type */
    VBICModel *VBICnextModel; /* pointer to next possible model in 
                                         linked list */
...
extern Shim::IfParm VBICpTable[];
extern int VBICpTSize;
extern Shim::IfParm VBICmPTable[];
extern int VBICmPTSize;
int VBICmParam(int param, Shim::IfValue *value, VBICModel *model);
int VBICparam(int param, Shim::IfValue *value, VBICInstance *inst, Shim::IfValue *select);
