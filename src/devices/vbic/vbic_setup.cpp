...
int
VBICsetup(Shim::Matrix *matrix, VBICModel *inModel, Shim::Ckt *ckt, int *states)
        ...
        for (here = model->VBICinstances; here != NULL ;
            ...
            *states += VBICnumStates;
            ...
            if(model->VBICextCollResist == 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->VBICname,"collector");
            ...
            if(model->VBICextBaseResist == 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->VBICname, "base");
                if(error) return(error);
            ...
            if(model->VBICemitterResist == 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->VBICname, "emitter");
                if(error) return(error);
            ...
            if(model->VBICsubstrateResist == 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->VBICname, "substrate");
                if(error) return(error);
                here->VBICsubsSINode = tmp->number;
                if (ckt->CKTcopyNodesets) {
                  if (Shim::CKTinst2Node(ckt,here,4,&tmpNode,&tmpName)==OK) {
                     if (tmpNode->nsGiven) {
                       tmp->nodeset=tmpNode->nodeset; 
                       tmp->nsGiven=tmpNode->nsGiven; 
                     }
            ...
            error = Shim::CKTmkVolt(ckt, &tmp, here->VBICname, "collCI");
            if(error) return(error);
            here->VBICcollCINode = tmp->number;  
            }

            if(here->VBICbaseBPNode == 0) {
            error = Shim::CKTmkVolt(ckt, &tmp, here->VBICname, "baseBP");
            if(error) return(error);
            here->VBICbaseBPNode = tmp->number;  
            }

            if(here->VBICbaseBINode == 0) {
            error = Shim::CKTmkVolt(ckt, &tmp, here->VBICname, "baseBI");
            if(error) return(error);
        ...
        model = model->VBICnextModel)
    {
                ...
                here=here->VBICnextInstance)
        {
            if (here->VBICcollCXNode
                && here->VBICcollCXNode != here->VBICcollNode)
            {
                Shim::CKTdltNNum(ckt, here->VBICcollCXNode);
                here->VBICcollCXNode = 0;
            }
            if (here->VBICbaseBXNode
                && here->VBICbaseBXNode != here->VBICbaseNode)
            {
                Shim::CKTdltNNum(ckt, here->VBICbaseBXNode);
                here->VBICbaseBXNode = 0;
            }
            if (here->VBICemitEINode
                && here->VBICemitEINode != here->VBICemitNode)
            {
                Shim::CKTdltNNum(ckt, here->VBICemitEINode);
                here->VBICemitEINode = 0;
            }
            if (here->VBICsubsSINode
                && here->VBICsubsSINode != here->VBICsubsNode)
            {
                Shim::CKTdltNNum(ckt, here->VBICsubsSINode);
                here->VBICsubsSINode = 0;
            }
            if (here->VBICcollCINode)
            {
                Shim::CKTdltNNum(ckt, here->VBICcollCINode);
                here->VBICcollCINode = 0;
            }
            if (here->VBICbaseBINode)
            {
                Shim::CKTdltNNum(ckt, here->VBICbaseBINode);
                here->VBICbaseBINode = 0;
            }
            if (here->VBICbaseBPNode)
            {
                Shim::CKTdltNNum(ckt, here->VBICbaseBPNode);
