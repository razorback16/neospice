...
int
VBICload(VBICModel *inModel, Shim::Ckt *ckt)
        /* actually load the current resistance value into the 
         * sparse matrix previously provided 
         */
{
    VBICModel *model = inModel;
    VBICInstance *here;
    double p[108]
    ...
    int error;
    /* int SenCond=0;  sensitivity stripped */
    ...
    for( ; model != NULL; model = model->VBICnextModel ) {
        ...
        for (here = model->VBICinstances; here != NULL ;
            ...
            /* Sensitivity analysis stripped for neospice */
            ...
            /* Sensitivity analysis block stripped for neospice */
...
next1:      
            ...
            if ( (!(ckt->CKTmode & MODEINITFIX))||(!(here->VBICoff))) {
                if (icheck == 1) {
                    ckt->CKTnoncon++;
                    ckt->CKTtroubleElt = (VBICInstance *) here;
                }
            }
...
next2:
            ...
            *(ckt->CKTstate0 + here->VBICire_Vre)   = Ire_Vre;  
            ...
            /* Sensitivity perturbation check stripped */
load:
            /*
            ...
            *(ckt->CKTrhs + here->VBICemitNode) +=  rhs_current;
            ckt->mat->add(here->VBICbaseBasePtr, gqbeo);
            ckt->mat->add(here->VBICemitEmitPtr, gqbeo);
            ckt->mat->add(here->VBICbaseEmitPtr, -gqbeo);
            ckt->mat->add(here->VBICemitBasePtr, -gqbeo);

            rhs_current = model->VBICtype * (*(ckt->CKTstate0 + here->VBICcqbco) - Vbc * gqbco);
            *(ckt->CKTrhs + here->VBICbaseNode) += -rhs_current;
            *(ckt->CKTrhs + here->VBICcollNode) +=  rhs_current;
            ckt->mat->add(here->VBICbaseBasePtr, gqbco);
            ckt->mat->add(here->VBICcollCollPtr, gqbco);
            ckt->mat->add(here->VBICbaseCollPtr, -gqbco);
            ckt->mat->add(here->VBICcollBasePtr, -gqbco);

            *(ckt->CKTrhs + here->VBICbaseBINode) += -cbcx;
            *(ckt->CKTrhs + here->VBICcollCXNode) +=  cbcx;
            ckt->mat->add(here->VBICbaseBIBaseBIPtr, gbcx);
            ckt->mat->add(here->VBICcollCXCollCXPtr, gbcx);
            ckt->mat->add(here->VBICbaseBICollCXPtr, -gbcx);
            ckt->mat->add(here->VBICcollCXBaseBIPtr, -gbcx);
...
*/
            ...
            *(ckt->CKTrhs + here->VBICbaseBINode) += -rhs_current;
            ckt->mat->add(here->VBICbaseBIBaseBIPtr, Ibe_Vbei);
            ckt->mat->add(here->VBICbaseBIEmitEIPtr, -Ibe_Vbei);
            *(ckt->CKTrhs + here->VBICemitEINode) +=  rhs_current;
            ckt->mat->add(here->VBICemitEIBaseBIPtr, -Ibe_Vbei);
            ckt->mat->add(here->VBICemitEIEmitEIPtr, Ibe_Vbei);
/*
c           Stamp element: Ibex
*/
            rhs_current = model->VBICtype * (Ibex - Ibex_Vbex*Vbex);
            *(ckt->CKTrhs + here->VBICbaseBXNode) += -rhs_current;
            ckt->mat->add(here->VBICbaseBXBaseBXPtr, Ibex_Vbex);
            ckt->mat->add(here->VBICbaseBXEmitEIPtr, -Ibex_Vbex);
            *(ckt->CKTrhs + here->VBICemitEINode) +=  rhs_current;
            ckt->mat->add(here->VBICemitEIBaseBXPtr, -Ibex_Vbex);
            ckt->mat->add(here->VBICemitEIEmitEIPtr, Ibex_Vbex);
...
*/
            ...
            *(ckt->CKTrhs + here->VBICcollCINode) += -rhs_current;
            ...
            ckt->mat->add(here->VBICcollCIEmitEIPtr, -Itzf_Vbei);
            ckt->mat->add(here->VBICcollCIBaseBIPtr, Itzf_Vbci);
            ckt->mat->add(here->VBICcollCICollCIPtr, -Itzf_Vbci);
            *(ckt->CKTrhs + here->VBICemitEINode) +=  rhs_current;
            ckt->mat->add(here->VBICemitEIBaseBIPtr, -Itzf_Vbei);
...
*/
            ...
            *(ckt->CKTrhs + here->VBICemitEINode) += -rhs_current;
            ...
            ckt->mat->add(here->VBICemitEIEmitEIPtr, -Itzr_Vbei);
            ckt->mat->add(here->VBICemitEIBaseBIPtr, Itzr_Vbci);
            ckt->mat->add(here->VBICemitEICollCIPtr, -Itzr_Vbci);
            *(ckt->CKTrhs + here->VBICcollCINode) +=  rhs_current;
            ckt->mat->add(here->VBICcollCIBaseBIPtr, -Itzr_Vbei);
...
*/
            ...
            *(ckt->CKTrhs + here->VBICbaseBINode) += -rhs_current;
            ckt->mat->add(here->VBICbaseBIBaseBIPtr, Ibc_Vbci);
            ckt->mat->add(here->VBICbaseBICollCIPtr, -Ibc_Vbci);
            ckt->mat->add(here->VBICbaseBIBaseBIPtr, Ibc_Vbei);
            ckt->mat->add(here->VBICbaseBIEmitEIPtr, -Ibc_Vbei);
            *(ckt->CKTrhs + here->VBICcollCINode) +=  rhs_current;
            ckt->mat->add(here->VBICcollCIBaseBIPtr, -Ibc_Vbci);
            ckt->mat->add(here->VBICcollCICollCIPtr, Ibc_Vbci);
            ckt->mat->add(here->VBICcollCIBaseBIPtr, -Ibc_Vbei);
            ckt->mat->add(here->VBICcollCIEmitEIPtr, Ibc_Vbei);
/*
c           Stamp element: Ibep
*/
            rhs_current = model->VBICtype * (Ibep - Ibep_Vbep*Vbep);
            *(ckt->CKTrhs + here->VBICbaseBXNode) += -rhs_current;
            ckt->mat->add(here->VBICbaseBXBaseBXPtr, Ibep_Vbep);
            ckt->mat->add(here->VBICbaseBXBaseBPPtr, -Ibep_Vbep);
            *(ckt->CKTrhs + here->VBICbaseBPNode) +=  rhs_current;
            ckt->mat->add(here->VBICbaseBPBaseBXPtr, -Ibep_Vbep);
            ckt->mat->add(here->VBICbaseBPBaseBPPtr, Ibep_Vbep);
/*
c           Stamp element: Rcx
*/
            ckt->mat->add(here->VBICcollCollPtr, Ircx_Vrcx);
            ckt->mat->add(here->VBICcollCXCollCXPtr, Ircx_Vrcx);
            ckt->mat->add(here->VBICcollCXCollPtr, -Ircx_Vrcx);
            ckt->mat->add(here->VBICcollCollCXPtr, -Ircx_Vrcx);
/*
...
*/
            ...
            *(ckt->CKTrhs + here->VBICcollCXNode) += -rhs_current;
            ckt->mat->add(here->VBICcollCXCollCXPtr, Irci_Vrci);
            ckt->mat->add(here->VBICcollCXCollCIPtr, -Irci_Vrci);
            ckt->mat->add(here->VBICcollCXBaseBIPtr, Irci_Vbci);
            ckt->mat->add(here->VBICcollCXCollCIPtr, -Irci_Vbci);
            ckt->mat->add(here->VBICcollCXBaseBIPtr, Irci_Vbcx);
            ckt->mat->add(here->VBICcollCXCollCXPtr, -Irci_Vbcx);
            *(ckt->CKTrhs + here->VBICcollCINode) +=  rhs_current;
            ckt->mat->add(here->VBICcollCICollCXPtr, -Irci_Vrci);
            ckt->mat->add(here->VBICcollCICollCIPtr, Irci_Vrci);
            ckt->mat->add(here->VBICcollCIBaseBIPtr, -Irci_Vbci);
            ckt->mat->add(here->VBICcollCICollCIPtr, Irci_Vbci);
            ckt->mat->add(here->VBICcollCIBaseBIPtr, -Irci_Vbcx);
            ckt->mat->add(here->VBICcollCICollCXPtr, Irci_Vbcx);
/*
c           Stamp element: Rbx
*/
            ckt->mat->add(here->VBICbaseBasePtr, Irbx_Vrbx);
            ckt->mat->add(here->VBICbaseBXBaseBXPtr, Irbx_Vrbx);
            ckt->mat->add(here->VBICbaseBXBasePtr, -Irbx_Vrbx);
            ckt->mat->add(here->VBICbaseBaseBXPtr, -Irbx_Vrbx);
/*
c           Stamp element: Irbi
*/
            rhs_current = model->VBICtype * (Irbi - Irbi_Vrbi*Vrbi - Irbi_Vbei*Vbei - Irbi_Vbci*Vbci);
            *(ckt->CKTrhs + here->VBICbaseBXNode) += -rhs_current;
            ckt->mat->add(here->VBICbaseBXBaseBXPtr, Irbi_Vrbi);
            ckt->mat->add(here->VBICbaseBXBaseBIPtr, -Irbi_Vrbi);
            ckt->mat->add(here->VBICbaseBXBaseBIPtr, Irbi_Vbei);
            ckt->mat->add(here->VBICbaseBXEmitEIPtr, -Irbi_Vbei);
            ckt->mat->add(here->VBICbaseBXBaseBIPtr, Irbi_Vbci);
            ckt->mat->add(here->VBICbaseBXCollCIPtr, -Irbi_Vbci);
            *(ckt->CKTrhs + here->VBICbaseBINode) +=  rhs_current;
            ckt->mat->add(here->VBICbaseBIBaseBXPtr, -Irbi_Vrbi);
            ckt->mat->add(here->VBICbaseBIBaseBIPtr, Irbi_Vrbi);
            ckt->mat->add(here->VBICbaseBIBaseBIPtr, -Irbi_Vbei);
            ckt->mat->add(here->VBICbaseBIEmitEIPtr, Irbi_Vbei);
            ckt->mat->add(here->VBICbaseBIBaseBIPtr, -Irbi_Vbci);
            ckt->mat->add(here->VBICbaseBICollCIPtr, Irbi_Vbci);
/*
c           Stamp element: Re
*/
            ckt->mat->add(here->VBICemitEmitPtr, Ire_Vre);
            ckt->mat->add(here->VBICemitEIEmitEIPtr, Ire_Vre);
            ckt->mat->add(here->VBICemitEIEmitPtr, -Ire_Vre);
            ckt->mat->add(here->VBICemitEmitEIPtr, -Ire_Vre);
/*
c           Stamp element: Irbp
*/
            rhs_current = model->VBICtype * (Irbp - Irbp_Vrbp*Vrbp - Irbp_Vbep*Vbep - Irbp_Vbci*Vbci);
            *(ckt->CKTrhs + here->VBICbaseBPNode) += -rhs_current;
            ckt->mat->add(here->VBICbaseBPBaseBPPtr, Irbp_Vrbp);
            ckt->mat->add(here->VBICbaseBPCollCXPtr, -Irbp_Vrbp);
            ckt->mat->add(here->VBICbaseBPBaseBXPtr, Irbp_Vbep);
            ckt->mat->add(here->VBICbaseBPBaseBPPtr, -Irbp_Vbep);
            ckt->mat->add(here->VBICbaseBPBaseBIPtr, Irbp_Vbci);
            ckt->mat->add(here->VBICbaseBPCollCIPtr, -Irbp_Vbci);
            *(ckt->CKTrhs + here->VBICcollCXNode) +=  rhs_current;
            ckt->mat->add(here->VBICcollCXBaseBPPtr, -Irbp_Vrbp);
            ckt->mat->add(here->VBICcollCXCollCXPtr, Irbp_Vrbp);
            ckt->mat->add(here->VBICcollCXBaseBXPtr, -Irbp_Vbep);
            ckt->mat->add(here->VBICcollCXBaseBPPtr, Irbp_Vbep);
            ckt->mat->add(here->VBICcollCXBaseBIPtr, -Irbp_Vbci);
            ckt->mat->add(here->VBICcollCXCollCIPtr, Irbp_Vbci);
/*
c           Stamp element: Ibcp
*/
            rhs_current = model->VBICtype * (Ibcp - Ibcp_Vbcp*Vbcp);
            *(ckt->CKTrhs + here->VBICsubsSINode) += -rhs_current;
            ckt->mat->add(here->VBICsubsSISubsSIPtr, Ibcp_Vbcp);
            ckt->mat->add(here->VBICsubsSIBaseBPPtr, -Ibcp_Vbcp);
            *(ckt->CKTrhs + here->VBICbaseBPNode) +=  rhs_current;
            ckt->mat->add(here->VBICbaseBPSubsSIPtr, -Ibcp_Vbcp);
            ckt->mat->add(here->VBICbaseBPBaseBPPtr, Ibcp_Vbcp);
/*
c           Stamp element: Iccp
*/
            rhs_current = model->VBICtype * (Iccp - Iccp_Vbep*Vbep - Iccp_Vbci*Vbci - Iccp_Vbcp*Vbcp);
            *(ckt->CKTrhs + here->VBICbaseBXNode) += -rhs_current;
            ckt->mat->add(here->VBICbaseBXBaseBXPtr, Iccp_Vbep);
            ckt->mat->add(here->VBICbaseBXBaseBPPtr, -Iccp_Vbep);
            ckt->mat->add(here->VBICbaseBXBaseBIPtr, Iccp_Vbci);
            ckt->mat->add(here->VBICbaseBXCollCIPtr, -Iccp_Vbci);
            ckt->mat->add(here->VBICbaseBXSubsSIPtr, Iccp_Vbcp);
            ckt->mat->add(here->VBICbaseBXBaseBPPtr, -Iccp_Vbcp);
            *(ckt->CKTrhs + here->VBICsubsSINode) +=  rhs_current;
            ckt->mat->add(here->VBICsubsSIBaseBXPtr, -Iccp_Vbep);
            ckt->mat->add(here->VBICsubsSIBaseBPPtr, Iccp_Vbep);
            ckt->mat->add(here->VBICsubsSIBaseBIPtr, -Iccp_Vbci);
            ckt->mat->add(here->VBICsubsSICollCIPtr, Iccp_Vbci);
            ckt->mat->add(here->VBICsubsSISubsSIPtr, -Iccp_Vbcp);
            ckt->mat->add(here->VBICsubsSIBaseBPPtr, Iccp_Vbcp);
/*
c           Stamp element: Rs
*/
            ckt->mat->add(here->VBICsubsSubsPtr, Irs_Vrs);
            ckt->mat->add(here->VBICsubsSISubsSIPtr, Irs_Vrs);
            ckt->mat->add(here->VBICsubsSISubsPtr, -Irs_Vrs);
            ckt->mat->add(here->VBICsubsSubsSIPtr, -Irs_Vrs);
