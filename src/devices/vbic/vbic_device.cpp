...
// set_state_ptrs
// ---------------------------------------------------------------------------
void VBICDevice::set_state_ptrs(double* s0, double* s1, double* s2, double* s3, int32_t base) {
    state0_ = s0;
    state1_ = s1;
    state2_ = s2;
    state3_ = s3;
...
double VBICDevice::compute_trunc(const IntegratorCtx& ctx,
    ...
    double dt_min = 1e30;
    static const int charge_offsets[] = {36, 39, 41, 43, 45, 48, 51, 60};  // charge states
