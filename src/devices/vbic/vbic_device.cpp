...
void VBICDevice::declare_internal_nodes(Circuit& ckt) {
    ...
    // Splice this instance as sole member so VBICsetup only processes *this*.
    ...
    int states = 0;
    int rc = VBICsetup(&shim_matrix, model_, &setup_ckt, &states);
    ...
    if (rc != Shim::OK) {
        throw std::runtime_error("VBICsetup failed with rc=" + std::to_string(rc));
    }
