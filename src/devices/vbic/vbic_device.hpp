...
struct VBICModelCard {
    ...
    VBICModelCard(const VBICModelCard&)            = delete;
    VBICModelCard& operator=(const VBICModelCard&) = delete;
};

class VBICDevice : public Device {
public:
    struct Geom {
        double area = 1.0;
        double m = 1.0;
    ...
    static std::unique_ptr<VBICDevice> make(
        std::string name, int32_t n_coll, int32_t n_base, int32_t n_emit, int32_t n_subs,
    ...
    /// Set initial condition voltages (called by parser when ic=VBE,VCE is present).
...
private:
    explicit VBICDevice(std::string name) : Device(std::move(name)) {}

    mutable vbic::VBICInstance inst_{};
    vbic::VBICModel* model_ = nullptr;
