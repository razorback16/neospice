...
int
JFET2load(JFET2Model *inModel, Shim::Ckt *ckt)
        ...
        if ( (fabs(cghat-*(ckt->CKTstate0 + here->JFET2cg)) 
		   ...
		   (fabs(cdhat-cd) > ckt->CKTreltol*
		    MAX(fabs(cdhat),fabs(cd))+ckt->CKTabstol)) {
                    ckt->CKTnoncon++;
		    ckt->CKTtroubleElt = (JFET2Instance *) here;
                }
            }
...
load:
            ...
            *(ckt->CKTrhs + here->JFET2sourcePrimeNode) +=
             ...
             *    load y matrix 
             */
            ckt->mat->add(here->JFET2drainDrainPrimePtr, m * (-gdpr));
            ckt->mat->add(here->JFET2gateDrainPrimePtr, m * (-ggd));
            ckt->mat->add(here->JFET2gateSourcePrimePtr, m * (-ggs));
            ckt->mat->add(here->JFET2sourceSourcePrimePtr, m * (-gspr));
            ckt->mat->add(here->JFET2drainPrimeDrainPtr, m * (-gdpr));
            ckt->mat->add(here->JFET2drainPrimeGatePtr, m * (gm-ggd));
            ckt->mat->add(here->JFET2drainPrimeSourcePrimePtr, m * (-gds-gm));
            ckt->mat->add(here->JFET2sourcePrimeGatePtr, m * (-ggs-gm));
            ckt->mat->add(here->JFET2sourcePrimeSourcePtr, m * (-gspr));
            ckt->mat->add(here->JFET2sourcePrimeDrainPrimePtr, m * (-gds));
            ckt->mat->add(here->JFET2drainDrainPtr, m * (gdpr));
            ckt->mat->add(here->JFET2gateGatePtr, m * (ggd+ggs));
            ckt->mat->add(here->JFET2sourceSourcePtr, m * (gspr));
            ckt->mat->add(here->JFET2drainPrimeDrainPrimePtr, m * (gdpr+gds+ggd));
            ckt->mat->add(here->JFET2sourcePrimeSourcePrimePtr, m * (gspr+gds+gm+ggs));
        }
    }
    return 0;
}
