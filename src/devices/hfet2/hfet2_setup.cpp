...
int HFET2setup(Shim::Matrix *matrix, HFET2Model *inModel, Shim::Ckt *ckt, int *states)
{
    ...
    for (here = model->HFET2instances; here != NULL; 
      ...
      *states += 13;
      ...
      if(model->HFET2rs != 0) {
        ...
        error = Shim::CKTmkVolt(ckt,&tmp,here->HFET2name,"source");
      ...
      if(model->HFET2rd != 0) {
        ...
        error = Shim::CKTmkVolt(ckt,&tmp,here->HFET2name,"drain");
        if(error) return(error);
...
int
HFET2unsetup(HFET2Model *inModel, Shim::Ckt *ckt)
{
    HFET2Model *model;
    HFET2Instance *here;
            ...
            model = model->HFET2nextModel)
    {
                ...
                here=here->HFET2nextInstance)
        {
            if (here->HFET2drainPrimeNode
                    && here->HFET2drainPrimeNode != here->HFET2drainNode)
            {
                Shim::CKTdltNNum(ckt, here->HFET2drainPrimeNode);
                here->HFET2drainPrimeNode = 0;
            }
            if (here->HFET2sourcePrimeNode
                    && here->HFET2sourcePrimeNode != here->HFET2sourceNode)
            {
                Shim::CKTdltNNum(ckt, here->HFET2sourcePrimeNode);
