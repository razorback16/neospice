...
void HFET2Device::declare_internal_nodes(Circuit& ckt) {
    ...
    // Splice this instance as sole member so HFET2setup only processes *this*.
    ...
    int states = 0;
    int rc = HFET2setup(&shim_matrix, model_, &setup_ckt, &states);
    ...
    if (rc != Shim::OK) {
        throw std::runtime_error("HFET2setup failed with rc=" + std::to_string(rc));
    }
...
// set_state_ptrs
// ---------------------------------------------------------------------------
void HFET2Device::set_state_ptrs(double* s0, double* s1, double* s2, int32_t base) {
    state0_ = s0;
    state1_ = s1;
    state2_ = s2;
    state_base_ = base;
...
double HFET2Device::compute_trunc(const IntegratorCtx& ctx,
    ...
    // Charge state variable offsets: qgs=9, qgd=11
