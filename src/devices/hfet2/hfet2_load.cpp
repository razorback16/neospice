...
static void hfeta2(HFET2Model *model, HFET2Instance *here, Shim::Ckt *ckt,
                  double vgs, double vds, double *cdrain, double *gm,
                  double *gds, double *capgs, double *capgd);


int HFET2load(HFET2Model *inModel, Shim::Ckt *ckt)
{
auto &mat = *ckt->mat;
...
load:
      ...
      *(ckt->CKTrhs + here->HFET2sourcePrimeNode) += m * (cdreq+ceqgs);
      ...
      mat.add(here->HFET2drainDrainPrimePtr, m * (-gdpr));
      mat.add(here->HFET2gateDrainPrimePtr, m * (-ggd));
      mat.add(here->HFET2gateSourcePrimePtr, m * (-ggs));
      mat.add(here->HFET2sourceSourcePrimePtr, m * (-gspr));
      mat.add(here->HFET2drainPrimeDrainPtr, m * (-gdpr));
      mat.add(here->HFET2drainPrimeGatePtr, m * (gm-ggd));
      mat.add(here->HFET2drainPriHFET2ourcePrimePtr, m * (-gds-gm));
      mat.add(here->HFET2sourcePrimeGatePtr, m * (-ggs-gm));
      mat.add(here->HFET2sourcePriHFET2ourcePtr, m * (-gspr));
      mat.add(here->HFET2sourcePrimeDrainPrimePtr, m * (-gds));
      mat.add(here->HFET2drainDrainPtr, m * (gdpr));
      mat.add(here->HFET2gateGatePtr, m * (ggd+ggs));
      mat.add(here->HFET2sourceSourcePtr, m * (gspr));
      mat.add(here->HFET2drainPrimeDrainPrimePtr, m * (gdpr+gds+ggd));
      mat.add(here->HFET2sourcePriHFET2ourcePrimePtr, m * (gspr+gds+gm+ggs));
    }
  }
  return 0;
