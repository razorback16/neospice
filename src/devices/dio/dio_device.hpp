#pragma once
// Adapter bridging the neospice Device interface to the UCB DIO code.

#include "devices/device.hpp"
#include "devices/dio/dio_def.hpp"
#include "devices/dio/dio_shim.hpp"
#include <memory>
#include <optional>
#include <utility>
#include <vector>

namespace neospice {

struct DIOModelCard {
    dio::DIOModel ucb{};   // aggregate UCB model fields

    DIOModelCard() = default;
    ~DIOModelCard();

    // Non-copyable / non-movable: UCB model is threaded with raw
    // pointers; copying would alias them.
    DIOModelCard(const DIOModelCard&)            = delete;
    DIOModelCard& operator=(const DIOModelCard&) = delete;
};

class DIODevice : public Device {
public:
    struct Geom {
        double area = 1.0;
        double pj = 0.0;
        double w = 0.0;
        double l = 0.0;
        double m = 1.0;
    };

    static std::unique_ptr<DIODevice> make(
        std::string name, int32_t n_pos, int32_t n_neg,
        const Geom& geom, DIOModelCard& shared_card);

    // Device interface
    void declare_internal_nodes(Circuit& ckt) override;
    void stamp_pattern(SparsityBuilder& builder) const override;
    void assign_offsets(const SparsityPattern& pattern) override;
    void evaluate(const std::vector<double>& voltages,
                  NumericMatrix& mat, std::vector<double>& rhs) override;
    void ac_stamp(const std::vector<double>& voltages,
                  NumericMatrix& G, NumericMatrix& C) override;

    int32_t state_vars() const override { return 5; }
    void set_state_ptrs(double* s0, double* s1, double* s2, int32_t base) override;
    double compute_trunc(const IntegratorCtx& ctx,
                         const SimOptions& opts) const override;
    bool device_converged() const override;
    std::optional<double> query_param(const std::string& name) const override;
    void reset_temp() override { temp_done_ = false; }

    std::vector<NoiseSource> noise_sources(
        double freq, const std::vector<double>& dc_solution) const override;

    void set_ic(double vd, bool vd_given);

private:
    explicit DIODevice(std::string name) : Device(std::move(name)) {}

    mutable dio::DIOInstance inst_{};
    dio::DIOModel* model_ = nullptr;

    std::vector<std::pair<int,int>> journal_;

    double* state0_ = nullptr;
    double* state1_ = nullptr;
    double* state2_ = nullptr;
    int32_t state_base_ = -1;

    mutable bool temp_done_ = false;
    mutable int last_noncon_ = 0;

    int32_t max_neo_node_ = -1;

    std::vector<double> ghost_voltages_;
    std::vector<double> ghost_rhs_;
};

} // namespace neospice
