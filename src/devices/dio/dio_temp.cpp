...
int
DIOtemp(DIOModel *inModel, Shim::Ckt *ckt)
{
...
#ifdef TRACE
                    Shim::report_error(Shim::ERR_WARNING, "%s: breakdown current increased to %g to resolve", here->DIOname, cbv);
                    Shim::report_error(Shim::ERR_WARNING,
...
#ifdef TRACE
                    Shim::report_error(Shim::ERR_WARNING, "%s: unable to match forward and reverse diode regions: bv = %g, ibv = %g", here->DIOname, xbv, xcbv);
#endif
                }
                matched:
