...
struct MOS3ModelCard {
    ...
    MOS3ModelCard(const MOS3ModelCard&)            = delete;
    MOS3ModelCard& operator=(const MOS3ModelCard&) = delete;
};
...
private:
    explicit MOS3Device(std::string name) : Device(std::move(name)) {}

    mutable mos3::MOS3Instance inst_{};
    mos3::MOS3Model* model_ = nullptr;
