...
void MOS3Device::declare_internal_nodes(Circuit& ckt) {
    ...
    // Splice this instance as sole member so MOS3setup only processes *this*.
    ...
    int states = 0;
    int rc = MOS3setup(&shim_matrix, model_, &setup_ckt, &states);
    ...
    if (rc != Shim::OK) {
        throw std::runtime_error("MOS3setup failed with rc=" + std::to_string(rc));
    }
