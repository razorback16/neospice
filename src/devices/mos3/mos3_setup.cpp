...
int
MOS3setup(Shim::Matrix *matrix, MOS3Model *inModel, Shim::Ckt *ckt, int *states)
         ...
         const char * tmpName;
            ...
            *states += MOS3NUMSTATES;
            ...
            if((model->MOS3drainResistance != 0 ||
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->MOS3name,"internal#drain");
            ...
            if((model->MOS3sourceResistance != 0 ||
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->MOS3name,"internal#source");
                if(error) return(error);
...
int
MOS3unsetup(MOS3Model *inModel, Shim::Ckt *ckt)
{
    MOS3Model *model;
    MOS3Instance *here;
                ...
                here=here->MOS3nextInstance)
	{
	    if (here->MOS3dNodePrime
		    && here->MOS3dNodePrime != here->MOS3dNode)
	    {
		Shim::CKTdltNNum(ckt, here->MOS3dNodePrime);
		here->MOS3dNodePrime= 0;
	    }
	    if (here->MOS3sNodePrime
		    && here->MOS3sNodePrime != here->MOS3sNode)
	    {
		Shim::CKTdltNNum(ckt, here->MOS3sNodePrime);
