...
int
HFETAsetup(Shim::Matrix *matrix, HFETAModel *inModel, Shim::Ckt *ckt, int *states)
        ...
        for (here = model->HFETAinstances; here != NULL ;
            ...
            *states += HFETAnumStates;
            ...
            if(model->HFETArs != 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->HFETAname,"source");
...
/* XXX: Applied AlansFixes  */               
            ...
            if(model->HFETArd != 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->HFETAname,"drain");
                if(error) return(error);
            ...
            if(model->HFETArg != 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->HFETAname,"gate");
                if(error) return(error);
            ...
            if(model->HFETArf != 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->HFETAname,"gd");
                if(error) return(error);
            ...
            if(model->HFETAri != 0) {
                ...
                error = Shim::CKTmkVolt(ckt,&tmp,here->HFETAname,"gs");
                if(error) return(error);
...
int
HFETAunsetup(HFETAModel *inModel, Shim::Ckt *ckt)
{
    HFETAModel *model;
    HFETAInstance *here;
            ...
            model = model->HFETAnextModel)
    {
                ...
                here=here->HFETAnextInstance)
        {
            if (here->HFETAdrainPrimeNode
                    && here->HFETAdrainPrimeNode != here->HFETAdrainNode)
            {
                Shim::CKTdltNNum(ckt, here->HFETAdrainPrimeNode);
                here->HFETAdrainPrimeNode = 0;
            }
            if (here->HFETAsourcePrimeNode
                    && here->HFETAsourcePrimeNode != here->HFETAsourceNode)
            {
                Shim::CKTdltNNum(ckt, here->HFETAsourcePrimeNode);
                here->HFETAsourcePrimeNode = 0;
            }
	    if (here->HFETAgatePrimeNode
        			&& here->HFETAgatePrimeNode != here->HFETAgateNode)
            {
        		Shim::CKTdltNNum(ckt, here->HFETAgatePrimeNode);
	    ...
	    if (here->HFETAdrainPrmPrmNode
        			&& here->HFETAdrainPrmPrmNode != here->HFETAdrainPrimeNode)
            {
        		Shim::CKTdltNNum(ckt, here->HFETAdrainPrmPrmNode);
        		here->HFETAdrainPrmPrmNode = 0;
            }
	    if (here->HFETAsourcePrmPrmNode
        			&& here->HFETAsourcePrmPrmNode != here->HFETAsourcePrimeNode)
            {
        		Shim::CKTdltNNum(ckt, here->HFETAsourcePrmPrmNode);
