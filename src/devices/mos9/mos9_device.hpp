...
struct MOS9ModelCard {
    ...
    MOS9ModelCard(const MOS9ModelCard&)            = delete;
    MOS9ModelCard& operator=(const MOS9ModelCard&) = delete;
};
...
public:
    ...
    static std::unique_ptr<MOS9Device> make(
...
private:
    explicit MOS9Device(std::string name) : Device(std::move(name)) {}

    mutable mos9::MOS9Instance inst_{};
    mos9::MOS9Model* model_ = nullptr;
