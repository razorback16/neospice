...
int
MOS9load(MOS9Model *inModel, Shim::Ckt *ckt)
...
innerline1000:;
            ...
            if ( (here->MOS9off == 0)  || 
                ...
                if (Check == 1) {
                    ckt->CKTnoncon++;
		    ckt->CKTtroubleElt = (MOS9Instance *) here;
                }
            }
...
/*printf("%g %g %g %g %g\n", -gcgs,-gcgd,0.0,-gcgs,0.0);*/
            ...
            ckt->mat->add(here->MOS9DdPtr, (here->MOS9drainConductance));
            ckt->mat->add(here->MOS9GgPtr, ((gcgd+gcgs+gcgb)));
            ckt->mat->add(here->MOS9SsPtr, (here->MOS9sourceConductance));
            ckt->mat->add(here->MOS9BbPtr, (here->MOS9gbd+here->MOS9gbs+gcgb));
            ckt->mat->add(here->MOS9DPdpPtr, (here->MOS9drainConductance+here->MOS9gds+
                here->MOS9gbd+xrev*(here->MOS9gm+here->MOS9gmbs)+gcgd));
            ckt->mat->add(here->MOS9SPspPtr, (here->MOS9sourceConductance+here->MOS9gds+
                here->MOS9gbs+xnrm*(here->MOS9gm+here->MOS9gmbs)+gcgs));
            ckt->mat->add(here->MOS9DdpPtr, (-here->MOS9drainConductance));
            ckt->mat->add(here->MOS9GbPtr, -(gcgb));
            ckt->mat->add(here->MOS9GdpPtr, -(gcgd));
            ckt->mat->add(here->MOS9GspPtr, -(gcgs));
            ckt->mat->add(here->MOS9SspPtr, (-here->MOS9sourceConductance));
            ckt->mat->add(here->MOS9BgPtr, -(gcgb));
            ckt->mat->add(here->MOS9BdpPtr, -(here->MOS9gbd));
            ckt->mat->add(here->MOS9BspPtr, -(here->MOS9gbs));
            ckt->mat->add(here->MOS9DPdPtr, (-here->MOS9drainConductance));
            ckt->mat->add(here->MOS9DPgPtr, ((xnrm-xrev)*here->MOS9gm-gcgd));
            ckt->mat->add(here->MOS9DPbPtr, (-here->MOS9gbd+(xnrm-xrev)*here->MOS9gmbs));
            ckt->mat->add(here->MOS9DPspPtr, (-here->MOS9gds-
                    xnrm*(here->MOS9gm+here->MOS9gmbs)));
            ckt->mat->add(here->MOS9SPgPtr, (-(xnrm-xrev)*here->MOS9gm-gcgs));
            ckt->mat->add(here->MOS9SPsPtr, (-here->MOS9sourceConductance));
