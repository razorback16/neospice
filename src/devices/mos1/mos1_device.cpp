...
MOS1Device::make(std::string name,
    ...
    // Geometry.
    inst.MOS1w = geom.W;
    inst.MOS1wGiven = geom.wGiven ? 1 : 0;
    inst.MOS1l = geom.L;
    inst.MOS1lGiven = geom.lGiven ? 1 : 0;
...
void MOS1Device::declare_internal_nodes(Circuit& ckt) {
    ucb_declare_internal_nodes<Shim::Matrix, Shim::Ckt>(
        ...
        [this](Shim::Matrix& m, Shim::Ckt& c) {
            UCB_SPLICE_INSTANCE(MOS1);
            int states = 0;
            int rc = MOS1setup(&m, model_, &c, &states);
            UCB_UNSPLICE_INSTANCE(MOS1);
            return rc;
        },
        "MOS1setup", journal_, max_neo_node_);
}
...
// set_state_ptrs
// ---------------------------------------------------------------------------
void MOS1Device::set_state_ptrs(double* s0, double* s1, double* s2, double* s3, int32_t base) {
    state0_ = s0;
    state1_ = s1;
    state2_ = s2;
    state3_ = s3;
