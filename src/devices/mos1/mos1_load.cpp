...
int
MOS1load(MOS1Model *inModel, Shim::Ckt *ckt)
...
#endif /*CAPBYPASS*/
    int SenCond;
    SenCond = 0;  // Sensitivity analysis not supported in neospice
...
*/
		 ...
		 (!(ckt->CKTmode & (MODEINITFIX|MODEINITSMSIG))) ){
                if (Check == 1) {
                    ckt->CKTnoncon++;
		    ckt->CKTtroubleElt = (MOS1Instance *) here;
                }
            }
/*
  
...
*/
	...
	bypass:
		...
		cdreq + ceqbs + model->MOS1type * ceqgs;
            ...
            ckt->mat->add(here->MOS1DdPtr, (here->MOS1drainConductance));
            ckt->mat->add(here->MOS1GgPtr, ((gcgd+gcgs+gcgb)));
            ckt->mat->add(here->MOS1SsPtr, (here->MOS1sourceConductance));
            ckt->mat->add(here->MOS1BbPtr, (here->MOS1gbd+here->MOS1gbs+gcgb));
            ckt->mat->add(here->MOS1DPdpPtr, (here->MOS1drainConductance+here->MOS1gds+
		 here->MOS1gbd+xrev*(here->MOS1gm+here->MOS1gmbs)+gcgd));
            ckt->mat->add(here->MOS1SPspPtr, (here->MOS1sourceConductance+here->MOS1gds+
		 here->MOS1gbs+xnrm*(here->MOS1gm+here->MOS1gmbs)+gcgs));
            ckt->mat->add(here->MOS1DdpPtr, (-here->MOS1drainConductance));
            ckt->mat->add(here->MOS1GbPtr, -(gcgb));
            ckt->mat->add(here->MOS1GdpPtr, -(gcgd));
            ckt->mat->add(here->MOS1GspPtr, -(gcgs));
            ckt->mat->add(here->MOS1SspPtr, (-here->MOS1sourceConductance));
            ckt->mat->add(here->MOS1BgPtr, -(gcgb));
            ckt->mat->add(here->MOS1BdpPtr, -(here->MOS1gbd));
            ckt->mat->add(here->MOS1BspPtr, -(here->MOS1gbs));
            ckt->mat->add(here->MOS1DPdPtr, (-here->MOS1drainConductance));
            ckt->mat->add(here->MOS1DPgPtr, ((xnrm-xrev)*here->MOS1gm-gcgd));
            ckt->mat->add(here->MOS1DPbPtr, (-here->MOS1gbd+(xnrm-xrev)*here->MOS1gmbs));
            ckt->mat->add(here->MOS1DPspPtr, (-here->MOS1gds-xnrm*
				     (here->MOS1gm+here->MOS1gmbs)));
            ckt->mat->add(here->MOS1SPgPtr, (-(xnrm-xrev)*here->MOS1gm-gcgs));
            ckt->mat->add(here->MOS1SPsPtr, (-here->MOS1sourceConductance));
