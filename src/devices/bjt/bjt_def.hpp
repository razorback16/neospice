...
struct BJTInstance {
    BJTModel *BJTmodPtr;    /* backpointer to model */
    BJTInstance *BJTnextInstance;   /* pointer to next instance of
                                             * current model*/
    const char *BJTname;  /* pointer to character string naming this instance */
    ...
    neospice::MatrixOffset BJTcolPrimeColPtr{-1};  /* pointer to sparse matrix at
                             * (collector prime,collector) */
    neospice::MatrixOffset BJTcolPrimeBasePrimePtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,base prime) */
    neospice::MatrixOffset BJTcolPrimeEmitPrimePtr{-1};    /* pointer to sparse matrix at
                             * (collector prime,emitter prime) */
    neospice::MatrixOffset BJTbasePrimeBasePtr{-1};    /* pointer to sparse matrix at
                             * (base prime,base ) */
    neospice::MatrixOffset BJTbasePrimeColPrimePtr{-1};    /* pointer to sparse matrix at
                             * (base prime,collector prime) */
    neospice::MatrixOffset BJTbasePrimeEmitPrimePtr{-1};   /* pointer to sparse matrix at
                             * (base primt,emitter prime) */
    neospice::MatrixOffset BJTemitPrimeEmitPtr{-1};    /* pointer to sparse matrix at
                             * (emitter prime,emitter) */
    neospice::MatrixOffset BJTemitPrimeColPrimePtr{-1};    /* pointer to sparse matrix at
                             * (emitter prime,collector prime) */
    neospice::MatrixOffset BJTemitPrimeBasePrimePtr{-1};   /* pointer to sparse matrix at
                             * (emitter prime,base prime) */
    neospice::MatrixOffset BJTcolColPtr{-1};   /* pointer to sparse matrix at
                             * (collector,collector) */
    neospice::MatrixOffset BJTbaseBasePtr{-1}; /* pointer to sparse matrix at
                             * (base,base) */
    neospice::MatrixOffset BJTemitEmitPtr{-1}; /* pointer to sparse matrix at
                             * (emitter,emitter) */
    neospice::MatrixOffset BJTcolPrimeColPrimePtr{-1}; /* pointer to sparse matrix at
                             * (collector prime,collector prime) */
    neospice::MatrixOffset BJTbasePrimeBasePrimePtr{-1};   /* pointer to sparse matrix at
                             * (base prime,base prime) */
    neospice::MatrixOffset BJTemitPrimeEmitPrimePtr{-1};   /* pointer to sparse matrix at
                             * (emitter prime,emitter prime) */
    neospice::MatrixOffset BJTsubstSubstPtr{-1};   /* pointer to sparse matrix at
                             * (substrate,substrate) */
    neospice::MatrixOffset BJTsubstConSubstPtr{-1};    /* pointer to sparse matrix at
                             * (Substrate connection, substrate) */
    neospice::MatrixOffset BJTsubstSubstConPtr{-1};    /* pointer to sparse matrix at
                             * (substrate, Substrate connection) */
    neospice::MatrixOffset BJTsubstConSubstConPtr{-1}; /* pointer to sparse matrix at
                             * (Substrate connection, Substrate connection) */
                            /* Substrate connection is either base prime *
                             * or collector prime depending on whether   *
                             * the device is VERTICAL or LATERAL         */
    neospice::MatrixOffset BJTbaseColPrimePtr{-1}; /* pointer to sparse matrix at
...
/* per model data */
struct BJTModel {          /* model structure for a bjt */
    int BJTmodType; /* type index of this device type */
    BJTModel *BJTnextModel; /* pointer to next possible model in
                                     * linked list */
    BJTInstance * BJTinstances; /* pointer to list of instances
                                 * that have this model */
    const char *BJTmodName; /* pointer to character string naming this model */

    /* --- end of generic struct BJTModel --- */
...
extern Shim::IfParm BJTpTable[];
extern int BJTpTSize;
extern Shim::IfParm BJTmPTable[];
extern int BJTmPTSize;
int BJTmParam(int param, Shim::IfValue *value, BJTModel *model);
int BJTparam(int param, Shim::IfValue *value, BJTInstance *inst, Shim::IfValue *select);
