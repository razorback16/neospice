...
#endif
#ifndef CONSTboltz
#define CONSTboltz 1.38064852e-23     // Boltzmann constant  (J/K)
#endif
#ifndef CONSTe
#define CONSTe 2.7182818284590452354 // Euler's number
#endif
#ifndef REFTEMP
#define REFTEMP 300.15               // 27 degC in Kelvin
#endif
#ifndef MAX_EXP_ARG
#define MAX_EXP_ARG 709.0            // ln(DBL_MAX) approx
#endif
...
// --- OFF flag (UCB instance "off" marker) -----------------------------------
#ifndef OFF
#define OFF 0
#endif
...
namespace Shim {
    ...
    struct Ckt {
        ...
        double *CKTrhsOp  = nullptr;   // operating-point RHS (alias for CKTrhsOld in DC)

        neospice::NumericMatrix *mat = nullptr;

        // Stub for convergence trouble reporting
        void *CKTtroubleElt = nullptr;

        // Stub for sensitivity analysis (BJT code gates on this pointer)
        struct SENinfo {
            int SENstatus = 0;
            int SENmode   = 0;
            int SENparms  = 0;
        };
        SENinfo *CKTsenInfo = nullptr;
    ...
    // --- Sensitivity analysis mode constants (from ngspice/sensdefs.h) -----
    constexpr int PERTURBATION = 1;
    constexpr int TRANSEN      = 2;
    constexpr int DCSEN        = 4;
    constexpr int ACSEN        = 8;
