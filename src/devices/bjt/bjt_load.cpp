...
int
BJTload(BJTModel *inModel, Shim::Ckt *ckt)
...
next1:      vtn=vt*here->BJTtemissionCoeffF;
            ...
            if ( (!(ckt->CKTmode & MODEINITFIX))||(!(here->BJToff))) {
                if (icheck == 1) {
                    ckt->CKTnoncon++;
                    ckt->CKTtroubleElt = (BJTInstance *) here;
                }
            }
...
load:
            ...
            *(ckt->CKTrhs + here->BJTsubstNode) += m * (-ceqsub);
            ...
            mat.add(here->BJTbasePrimeColPrimePtr, m * (-gmu-geqcb));
            mat.add(here->BJTbasePrimeEmitPrimePtr, m * (-gpi));
            mat.add(here->BJTemitPrimeEmitPtr, m * (-gepr));
            mat.add(here->BJTemitPrimeColPrimePtr, m * (-go+geqcb));
            mat.add(here->BJTemitPrimeBasePrimePtr, m * (-gpi-gm-geqcb));
            mat.add(here->BJTsubstSubstPtr, m * (geqsub));
            mat.add(here->BJTsubstConSubstPtr, m * (-geqsub));
