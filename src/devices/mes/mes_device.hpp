...
struct MESModelCard {
    ...
    MESModelCard(const MESModelCard&)            = delete;
    MESModelCard& operator=(const MESModelCard&) = delete;
};
...
public:
    ...
    static std::unique_ptr<MESDevice> make(
...
private:
    explicit MESDevice(std::string name) : Device(std::move(name)) {}

    mutable mes::MESInstance inst_{};
    mes::MESModel* model_ = nullptr;
