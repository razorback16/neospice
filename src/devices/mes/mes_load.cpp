...
int
MESload(MESModel *inModel, Shim::Ckt *ckt)
{
auto &mat = *ckt->mat;
		...
		if((icheck == 1) ||
		   ...
		   (fabs(cdhat-cd) > ckt->CKTreltol*
		    MAX(fabs(cdhat),fabs(cd))+ckt->CKTabstol)) {
                    ckt->CKTnoncon++;
		    ckt->CKTtroubleElt = (MESInstance *) here;
                }
            }
...
load:
            ...
            *(ckt->CKTrhs + here->MESsourcePrimeNode) +=
             ...
             *    load y matrix 
             */
            mat.add(here->MESdrainDrainPrimePtr, m * (-gdpr));
            mat.add(here->MESgateDrainPrimePtr, m * (-ggd));
            mat.add(here->MESgateSourcePrimePtr, m * (-ggs));
            mat.add(here->MESsourceSourcePrimePtr, m * (-gspr));
            mat.add(here->MESdrainPrimeDrainPtr, m * (-gdpr));
            mat.add(here->MESdrainPrimeGatePtr, m * (gm-ggd));
            mat.add(here->MESdrainPrimeSourcePrimePtr, m * (-gds-gm));
            mat.add(here->MESsourcePrimeGatePtr, m * (-ggs-gm));
            mat.add(here->MESsourcePrimeSourcePtr, m * (-gspr));
            mat.add(here->MESsourcePrimeDrainPrimePtr, m * (-gds));
            mat.add(here->MESdrainDrainPtr, m * (gdpr));
            mat.add(here->MESgateGatePtr, m * (ggd+ggs));
            mat.add(here->MESsourceSourcePtr, m * (gspr));
            mat.add(here->MESdrainPrimeDrainPrimePtr, m * (gdpr+gds+ggd));
            mat.add(here->MESsourcePrimeSourcePrimePtr, m * (gspr+gds+gm+ggs));
        }
    }
    return 0;
}
