...
/* define local variavles */
  ...
  double Ps_delta0 = 0.10 ;
    ...
    // Constants
    Vbi_DEP = here->HSMHV_Vbipn ;
    q_Ndepm = C_QE * here->HSMHV_ndepm ;
    Ndepm2  = here->HSMHV_ndepm * here->HSMHV_ndepm ;
    q_Ndepm_esi = C_QE * here->HSMHV_ndepm * C_ESI ;
    q_Nsub = C_QE * here->HSMHV_nsub ;
    C_QE2  = C_QE * C_QE ;
    C_ESI2 = C_ESI * C_ESI ;
    Tn2    = model->HSMHV_tndep * model->HSMHV_tndep ;
    ...
    NdepmpNsub  = here->HSMHV_ndepm / here->HSMHV_nsub ;
    ...
    ps_conv23 = ps_conv2 * 1000.0 ;
      ...
      *------------------*/
      ...
      afact = Cox * Cox / here->HSMHV_cnst0 / here->HSMHV_cnst0 ;
      afact2 = afact / here->HSMHV_nin / here->HSMHV_nin * Ndepm2 ;
      W_bsub0 = sqrt(2.0e0 * C_ESI / C_QE * here->HSMHV_nsub / (here->HSMHV_nsub
              + here->HSMHV_ndepm) / here->HSMHV_ndepm * ( - Vbscl + Vbi_DEP)) ;

      if( W_bsub0 > model->HSMHV_tndep ) {

        Vgp0 = 0.0;

        W_b0 = model->HSMHV_tndep ;
        phi_b0_DEP = 0.0 ;
        ...
        for ( lp_s0 = 1 ; lp_s0 <= lp_se_max + 1 ; lp_s0 ++ ) {
          ...
          Fn_SU_CP( W_b0 , W_b0 , model->HSMHV_tndep , 1e-8, 2 , T0 )
        ...
        W_sub0 = model->HSMHV_tndep * NdepmpNsub ;
      ...
      T1 = C_2ESI_q_Ndepm * ( Psbmax - ( - here->HSMHV_Pb2n + Vbscl)) ;
      if ( T1 > 0.0 ) {
        vthn = - here->HSMHV_Pb2n + Vbscl - sqrt(T1) / Cox ;
      } else {
        vthn = - here->HSMHV_Pb2n + Vbscl ;
      }
      ...
      /* primary value */
        ...
        /* depletion and inversion region */
         ...
         if( Vgp > vthn ) {
          /* depletion */
           ...
           if( W_s0 + W_b0 > model->HSMHV_tndep ) {
             for ( lp_s0 = 1 ; lp_s0 <= lp_se_max + 1 ; lp_s0 ++ ) {

               y0 = W_s0 + W_b0 - model->HSMHV_tndep ;

               dydPsm = C_ESI / q_Ndepm / W_s0 
                  + C_ESI / q_Ndepm * ( 1.0 - (here->HSMHV_ndepm
                  / here->HSMHV_nsub) / ( 1.0 + (NdepmpNsub))) / W_b0 ;
       ...
       for ( lp_s0 = 1 ; lp_s0 <= lp_se_max + 1 ; lp_s0 ++ ) {
         ...
         Fn_SU_CP( W_b0 , W_b0 , model->HSMHV_tndep, 1e-8, 2 , T8 )
         W_sub0 = sqrt(C_2ESIpq_Nsub * (phi_j0_DEP - Vbscl + Vbi_DEP) ) ;
         Q_b0_dep = W_b0 * q_Ndepm ;
         ...
         phib_ref = (4.0 * phi_j0_DEP * phi_j0_DEP * C_ESI2 - 8.0 * phi_j0_DEP * C_ESI2 * phi_s0_DEP
              ...
              + Ndepm2 * C_QE2 * model->HSMHV_tndep
              * Tn2 * model->HSMHV_tndep) / T10 ;
         ...
         if( phi_s0_DEP >= phi_b0_DEP ) { 
           Q_s0 = - here->HSMHV_cnst0 * sqrt(T2 - 1.0 - T1 + 1e-15) ;
           Q_s0_dPs = 0.5 * here->HSMHV_cnst0 * here->HSMHV_cnst0 / Q_s0 * (beta * T2 - beta ) ;
           Q_s0_dPb = - Q_s0_dPs ;
         } else {
           T3 = exp( - beta * (phi_s0_DEP - Vbscl)) ;
           T4 = exp( - beta * (phi_b0_DEP - Vbscl)) ;
           Q_s0 = here->HSMHV_cnst0 * sqrt(T2 - 1.0 - T1 + 1e-15 + here->HSMHV_cnst1 * (T3 - T4) ) ;
           T5 = 0.5 * here->HSMHV_cnst0 * here->HSMHV_cnst0 / Q_s0 ;
           Q_s0_dPs = T5 * (beta * T2 - beta + here->HSMHV_cnst1 * ( - beta * T3) ) ;
           Q_s0_dPb = T5 * ( - beta * T2 + beta + here->HSMHV_cnst1 * beta * T4 ) ;
         }
      ...
      /* caluculate derivative */
       ...
       if( phi_s0_DEP >= phi_b0_DEP ) {
         ...
         Q_s0_dT   = - cnst0_dT * sqrt(T2 - 1.0 - T1 + 1e-15)
                   - here->HSMHV_cnst0 / 2.0 / sqrt(T2 - 1.0 - T1 + 1e-15) * ((phi_s0_DEP - phi_b0_DEP) * T2 * beta_dT
                   - (phi_s0_DEP - phi_b0_DEP) * beta_dT) ;
       } else {
         T3 = exp( - beta * (phi_s0_DEP - Vbscl)) ;
         T4 = exp( - beta * (phi_b0_DEP - Vbscl)) ;
         T5 = sqrt(T2 - 1.0 - T1 + 1e-15 + here->HSMHV_cnst1 * (T3 - T4)) ;
         Q_s0_dVbs = here->HSMHV_cnst0 / 2.0 / T5 * 
                  (here->HSMHV_cnst1 * (beta * T3 * Vbscl_dVbs - beta * T4 * Vbscl_dVbs) ) ;
         Q_s0_dT   = cnst0_dT * T5
                  + here->HSMHV_cnst0 / 2.0 / T5 * 
                   ((phi_s0_DEP - phi_b0_DEP) * T2 * beta_dT - (phi_s0_DEP - phi_b0_DEP) * beta_dT
                   + cnst1_dT * (T3 - T4)
                   + here->HSMHV_cnst1 * ( - (phi_s0_DEP - Vbscl) * T3 * beta_dT + beta * T3 * Vbscl_dT
       ...
       if( W_bsub0 > model->HSMHV_tndep && depmode !=2 ) {
...
///      Qg = Cox * (Vgp - phi_s0_DEP) ;
         ...
         W_b0 = sqrt(C_2ESIpq_Ndepm * (phi_b0_DEP - phi_j0_DEP) ) ;
         Fn_SU_CP( T9 , W_b0 , model->HSMHV_tndep, 1e-8, 2 , T4 )
        ...
        /* derivative */
         Q_s0_dVgs = - here->HSMHV_cnst0 * T3_dVgs ;
         Q_s0_dVds = - here->HSMHV_cnst0 * T3_dVds ;
         Q_s0_dVbs = - here->HSMHV_cnst0 * T3_dVbs ;
         Q_s0_dT = - cnst0_dT * T3
                 - here->HSMHV_cnst0 * T3_dT ;
         ...
         T3 = exp( - beta * (phi_s0_DEP - Vbscl)) ;
         T4 = exp( - beta * (phi_b0_DEP - Vbscl)) ;
         T5 = sqrt(T2 - 1.0 - T1 + here->HSMHV_cnst1 * (T3 - T4) + 1e-15) ;
         Q_s0 = here->HSMHV_cnst0 * T5 ;
         ...
         T5_dVgs = (T2_dVgs - T1_dVgs + here->HSMHV_cnst1 * (T3_dVgs - T4_dVgs)) / 2.0 / T5 ;
         T5_dVds = (T2_dVds - T1_dVds + here->HSMHV_cnst1 * (T3_dVds - T4_dVds)) / 2.0 / T5 ;
         T5_dVbs = (T2_dVbs - T1_dVbs + here->HSMHV_cnst1 * (T3_dVbs - T4_dVbs)) / 2.0 / T5 ;
         T5_dT   = (T2_dT - T1_dT + here->HSMHV_cnst1 * (T3_dT - T4_dT) + cnst1_dT * (T3 - T4)) / 2.0 / T5 ;
 
         Q_s0_dVgs = here->HSMHV_cnst0 * T5_dVgs ;
         Q_s0_dVds = here->HSMHV_cnst0 * T5_dVds ;
         Q_s0_dVbs = here->HSMHV_cnst0 * T5_dVbs ;
         Q_s0_dT   = here->HSMHV_cnst0 * T5_dT + cnst0_dT * T5 ;
         ...
         if( W_bsub0 > model->HSMHV_tndep && depmode !=2 ) {
           ...
           T3 = exp( - beta * (phi_s0_DEP - Vbscl)) ;
           T4 = exp( - beta * (phi_b0_DEP - Vbscl)) ;
           T5 = sqrt( - T1 + here->HSMHV_cnst1 * (T3 - T4)) ;
           Q_sub0 = here->HSMHV_cnst0 * T5 - here->HSMHV_cnst0 * sqrt( - T1)  ;
           T6 = sqrt(T2 - 1.0e0 - T1 + 1e-15) ;
           Q_s0_dep = here->HSMHV_cnst0 * T6 ;

           Q_sub0_dVgs = here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_s0_DEP_dVgs - phi_b0_DEP_dVgs)
              + here->HSMHV_cnst1 * ( - beta * T3 * phi_s0_DEP_dVgs + beta * T4 * phi_b0_DEP_dVgs))
              - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_s0_DEP_dVgs - phi_b0_DEP_dVgs)) ;
           Q_sub0_dVds = here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_s0_DEP_dVds - phi_b0_DEP_dVds)
              + here->HSMHV_cnst1 * ( - beta * T3 * phi_s0_DEP_dVds + beta * T4 * phi_b0_DEP_dVds))
              - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_s0_DEP_dVds - phi_b0_DEP_dVds)) ;
           Q_sub0_dVbs = here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_s0_DEP_dVbs - phi_b0_DEP_dVbs)
              + here->HSMHV_cnst1 * ( - beta * T3 * (phi_s0_DEP_dVbs - Vbscl_dVbs) + beta * T4 * (phi_b0_DEP_dVbs - Vbscl_dVbs)))
              - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_s0_DEP_dVbs - phi_b0_DEP_dVbs)) ;
           Q_sub0_dT = cnst0_dT * T5 - cnst0_dT * sqrt( - T1)
              + here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_s0_DEP_dT - phi_b0_DEP_dT) - beta_dT * (phi_s0_DEP - phi_b0_DEP)
              + cnst1_dT * (T3 - T4)
              + here->HSMHV_cnst1 * ( - beta * T3 * (phi_s0_DEP_dT - Vbscl_dT) - beta_dT * (phi_s0_DEP - Vbscl) * T3
                                  + beta * T4 * (phi_b0_DEP_dT - Vbscl_dT) + beta_dT * (phi_b0_DEP - Vbscl) * T4))
              - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_s0_DEP_dT - phi_b0_DEP_dT) - beta_dT * (phi_s0_DEP - phi_b0_DEP)) ;

           Q_s0_dep_dVgs = here->HSMHV_cnst0 / 2.0 / T6 * beta * (phi_s0_DEP_dVgs - phi_b0_DEP_dVgs) * (T2 - 1) ;
           Q_s0_dep_dVds = here->HSMHV_cnst0 / 2.0 / T6 * beta * (phi_s0_DEP_dVds - phi_b0_DEP_dVds) * (T2 - 1) ;
           Q_s0_dep_dVbs = here->HSMHV_cnst0 / 2.0 / T6 * beta * (phi_s0_DEP_dVbs - phi_b0_DEP_dVbs) * (T2 - 1) ;
           Q_s0_dep_dT = cnst0_dT * T6
                 + here->HSMHV_cnst0 / 2.0 / T6 * 
...
///      Qg = Cox * (Vgp - phi_s0_DEP) ;
         ...
         W_b0 = sqrt(C_2ESIpq_Ndepm * (T2) ) ; 
         Fn_SU_CP( T9 , W_b0 , model->HSMHV_tndep, 1e-8, 2 , T3 )
         W_sub0 = sqrt(C_2ESIpq_Nsub * (phi_j0_DEP - Vbscl + Vbi_DEP) ) ;
         Q_b0_dep = T9 * q_Ndepm ;
         Q_sub0_dep = - W_sub0 * q_Nsub ;
       ...
       T5 = sqrt(T4) ;
       Q_n0_cur = - here->HSMHV_cnst0 * T5 ;
       Q_n0_cur_dVgs = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dVgs ;
       Q_n0_cur_dVds = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dVds ;
       Q_n0_cur_dVbs = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dVbs ;
       Q_n0_cur_dT   = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dT
                      - cnst0_dT * T5 ;
       ...
       T5 = sqrt(T4) ;
       T5_dT = 0.5 / T5 * T4_dT ;
       Qn_delta = here->HSMHV_cnst0 * T5 ;

       Qn_delta_dT =  cnst0_dT * T5 + here->HSMHV_cnst0 * T5_dT ;
     ...
     /* Vdseff (begin) */
       ...
       if( Vds > 1e-3 ) {
         ...
         here->HSMHV_qnsub_esi = q_Ndepm_esi ;
	 ...
	 Q_nL_cur_dT   = Q_n0_cur_dT ;
         ...
         W_bsubL = sqrt(C_2ESIpq_Ndepm * here->HSMHV_nsub / (here->HSMHV_nsub + here->HSMHV_ndepm) * (Vds - Vbscl + Vbi_DEP)) ;
        ...
        *------------------*/
        ...
        /* fully depleted case */
         if( W_bsubL > model->HSMHV_tndep ) {

           Vgp0 = Vds ;
           W_bL = model->HSMHV_tndep ;
           phi_bL_DEP = Vds ;
           phi_bL_DEP_lim = Vds ;
           ...
           for ( lp_s0 = 1 ; lp_s0 <= lp_se_max + 1 ; lp_s0 ++ ) {
             ...
             Fn_SU_CP( W_bL , W_bL , model->HSMHV_tndep , 1e-8, 2 , T0 )
           ...
           W_subL = model->HSMHV_tndep * NdepmpNsub ;
        ...
        *------------------*/
        ...
        /* depletion & inversion */
          ...
          /* depletion */
           if( Vgp > vthn ) {
             ...
             if( W_sL + W_bL > model->HSMHV_tndep ) {
               for ( lp_s0 = 1 ; lp_s0 <= lp_se_max + 1 ; lp_s0 ++ ) {

                 y0 = W_sL + W_bL - model->HSMHV_tndep ;

                 dydPsm = C_ESI / q_Ndepm / W_sL 
                          + C_ESI / q_Ndepm * ( 1.0 - (here->HSMHV_ndepm
                          / here->HSMHV_nsub) / ( 1.0 + (NdepmpNsub))) / W_bL ;
        ...
        /* accumulation */
         ...
         for ( lp_s0 = 1 ; lp_s0 <= lp_se_max + 1 ; lp_s0 ++ ) {
           ...
           Fn_SU_CP( W_bL , W_bL , model->HSMHV_tndep , 1e-8, 2 , T8 )
           W_subL = sqrt(C_2ESIpq_Nsub * (phi_jL_DEP - Vbscl + Vbi_DEP) ) ;
           Q_bL_dep = W_bL * q_Ndepm ;
           ...
           if( phi_sL_DEP >= phi_bL_DEP ) {
             Q_sL = - here->HSMHV_cnst0 * sqrt(T2 - 1.0 - T1 + 1e-15) ;
             Q_sL_dPs = 0.5 * here->HSMHV_cnst0 * here->HSMHV_cnst0 / Q_sL * (beta * T2 - beta) ;
             Q_sL_dPb = - Q_sL_dPs ;
           } else {
             T3 = exp( - beta * (phi_sL_DEP - Vbscl)) ;
             T4 = exp( - beta * (phi_bL_DEP - Vbscl)) ;
             Q_sL = here->HSMHV_cnst0 * sqrt(T2 - 1.0 - T1 + here->HSMHV_cnst1 * (T3 - T4) + 1e-15) ;
             T5 = 0.5 * here->HSMHV_cnst0 * here->HSMHV_cnst0 / Q_sL ;
             Q_sL_dPs = T5 * (beta * T2 - beta + here->HSMHV_cnst1 * ( - beta * T3) ) ;
             Q_sL_dPb = T5 * ( - beta * T2 + beta + here->HSMHV_cnst1 * beta * T4 ) ;
           }
        ...
        /* caluculate derivative */
         ...
         if( phi_sL_DEP >= phi_bL_DEP ) {
           ...
           Q_sL_dT   = - cnst0_dT * sqrt(T2 - 1.0 - T1 + 1e-15)
                   - here->HSMHV_cnst0 / 2.0 / sqrt(T2 - 1.0 - T1 + 1e-15) * ((phi_sL_DEP - phi_bL_DEP) * T2 * beta_dT
                   - (phi_sL_DEP - phi_bL_DEP) * beta_dT) ;
         } else {
           T3 = exp( - beta * (phi_sL_DEP - Vbscl)) ;
           T4 = exp( - beta * (phi_bL_DEP - Vbscl)) ;
           T5 = sqrt(T2 - 1.0 - T1 + 1e-15 + here->HSMHV_cnst1 * (T3 - T4)) ;
           Q_sL_dVbs = here->HSMHV_cnst0 / 2.0 / T5 * 
                  (here->HSMHV_cnst1 * (beta * T3 * Vbscl_dVbs - beta * T4 * Vbscl_dVbs) ) ;
           Q_sL_dT   = cnst0_dT * T5
                  + here->HSMHV_cnst0 / 2.0 / T5 * 
                   ((phi_sL_DEP - phi_bL_DEP) * T2 * beta_dT - (phi_sL_DEP - phi_bL_DEP) * beta_dT
                   + cnst1_dT * (T3 - T4)
                   + here->HSMHV_cnst1 * ( - (phi_sL_DEP - Vbscl) * T3 * beta_dT + beta * T3 * Vbscl_dT
         ...
         if( W_bsubL > model->HSMHV_tndep && depmode !=2 ) {
...
///        Qg = Cox * (Vgp - phi_sL_DEP) ;
           ...
           W_bL = sqrt(C_2ESIpq_Ndepm * (phi_bL_DEP - phi_jL_DEP) ) ;
           Fn_SU_CP( T9 , W_bL , model->HSMHV_tndep, 1e-8, 2 , T4 )
          ...
          /* derivative */
           Q_sL_dVgs = - here->HSMHV_cnst0 * T3_dVgs ;
           Q_sL_dVds = - here->HSMHV_cnst0 * T3_dVds ;
           Q_sL_dVbs = - here->HSMHV_cnst0 * T3_dVbs ;
           Q_sL_dT = - cnst0_dT * T3
                 - here->HSMHV_cnst0 * T3_dT ;
           ...
           Q_sL_dVgs = here->HSMHV_cnst0 * T5_dVgs ;
           Q_sL_dVds = here->HSMHV_cnst0 * T5_dVds ;
           Q_sL_dVbs = here->HSMHV_cnst0 * T5_dVbs ;
           Q_sL_dT   = here->HSMHV_cnst0 * T5_dT + cnst0_dT * T5 ;
           ...
           if( W_bsubL > model->HSMHV_tndep && depmode !=2 ) {
             ...
             T4 = exp( - beta * (phi_bL_DEP - Vbscl)) ;
             T5 = sqrt( - T1 + here->HSMHV_cnst1 * (T3 - T4)) ;
             Q_subL = here->HSMHV_cnst0 * T5 - here->HSMHV_cnst0 * sqrt( - T1)  ;
             T6 = sqrt(T2 - 1.0e0 - T1 + 1e-15) ;
             Q_sL_dep = here->HSMHV_cnst0 * T6 ;

             Q_subL_dVgs = here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_sL_DEP_dVgs - phi_bL_DEP_dVgs)
                + here->HSMHV_cnst1 * ( - beta * T3 * phi_sL_DEP_dVgs + beta * T4 * phi_bL_DEP_dVgs))
                - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_sL_DEP_dVgs - phi_bL_DEP_dVgs)) ;
             Q_subL_dVds = here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_sL_DEP_dVds - phi_bL_DEP_dVds)
                + here->HSMHV_cnst1 * ( - beta * T3 * phi_sL_DEP_dVds + beta * T4 * phi_bL_DEP_dVds))
                - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_sL_DEP_dVds - phi_bL_DEP_dVds)) ;
             Q_subL_dVbs = here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_sL_DEP_dVbs - phi_bL_DEP_dVbs)
                + here->HSMHV_cnst1 * ( - beta * T3 * (phi_sL_DEP_dVbs - Vbscl_dVbs) + beta * T4 * (phi_bL_DEP_dVbs - Vbscl_dVbs)))
                - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_sL_DEP_dVbs - phi_bL_DEP_dVbs)) ;
             Q_subL_dT = cnst0_dT * T5 - cnst0_dT * sqrt( - T1)
                + here->HSMHV_cnst0 / 2.0 / T5 * ( - beta * (phi_sL_DEP_dT - phi_bL_DEP_dT) - beta_dT * (phi_sL_DEP - phi_bL_DEP)
                + cnst1_dT * (T3 - T4)
                + here->HSMHV_cnst1 * ( - beta * T3 * (phi_sL_DEP_dT - Vbscl_dT) - beta_dT * (phi_sL_DEP - Vbscl) * T3
                                  + beta * T4 * (phi_bL_DEP_dT - Vbscl_dT) + beta_dT * (phi_bL_DEP - Vbscl) * T4))
                - here->HSMHV_cnst0 / 2.0 / sqrt( - T1) * ( - beta * (phi_sL_DEP_dT - phi_bL_DEP_dT) - beta_dT * (phi_sL_DEP - phi_bL_DEP)) ;

             Q_sL_dep_dVgs = here->HSMHV_cnst0 / 2.0 / T6 * beta * (phi_sL_DEP_dVgs - phi_bL_DEP_dVgs) * (T2 - 1) ;
             Q_sL_dep_dVds = here->HSMHV_cnst0 / 2.0 / T6 * beta * (phi_sL_DEP_dVds - phi_bL_DEP_dVds) * (T2 - 1) ;
             Q_sL_dep_dVbs = here->HSMHV_cnst0 / 2.0 / T6 * beta * (phi_sL_DEP_dVbs - phi_bL_DEP_dVbs) * (T2 - 1) ;
...
///        Qg = Cox * (Vgp - phi_sL_DEP) ;
           ...
           Fn_SL_CP( T2 , T1 , 0.0 , 0.1, 2 , T0 )
           W_bL = sqrt(C_2ESIpq_Ndepm * (T2) ) ;
           Fn_SU_CP( T9 , W_bL , model->HSMHV_tndep, 1e-8, 2 , T3 )
           W_subL = sqrt(C_2ESIpq_Nsub * (phi_jL_DEP - Vbscl + Vbi_DEP) ) ;
           Q_bL_dep = T9 * q_Ndepm ;
           Q_subL_dep = - W_subL * q_Nsub ;
         ...
         T5 = sqrt(T4) ;
         Q_nL_cur = - here->HSMHV_cnst0 * T5 ;
         Q_nL_cur_dVgs = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dVgs ;
         Q_nL_cur_dVds = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dVds ;
         Q_nL_cur_dVbs = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dVbs ;
         Q_nL_cur_dT   = - here->HSMHV_cnst0 / 2.0 / T5 * T4_dT
                        - cnst0_dT * T5 ;
     ...
     //*-----------------//
       ...
       if( Vdsorg > 1e-3 ) {
         ...
         T0 = Vgp + model->HSMHV_depvdsef1 - beta_inv - Vbsz ;
         T0_dVg = Vgp_dVgs ;
         T0_dVd = Vgp_dVds - Vbsz_dVds ;
         T0_dVb = Vgp_dVbs - Vbsz_dVbs ;
         T0_dT = Vgp_dT - beta_inv_dT - Vbsz_dT ;
         ...
         T10 = Vgp + model->HSMHV_depvdsef1 + T2 * ( 1.0e0 - T3 ) ;
         T10 = T10 * model->HSMHV_depvdsef2 ;
         T10_dVb = (Vgp_dVbs + T2_dVb * ( 1.0e0 - T3 ) - T2 * T3_dVb)
                   * model->HSMHV_depvdsef2 ;
         T10_dVd = (Vgp_dVds + T2_dVd * ( 1.0e0 - T3 ) - T2 * T3_dVd)
                   * model->HSMHV_depvdsef2 ; 
         T10_dVg = (Vgp_dVgs + T2_dVg * ( 1.0e0 - T3 ) - T2 * T3_dVg) 
                   * model->HSMHV_depvdsef2 ;
         ...
         Fn_SL_CP( T10 , T10 , model->HSMHV_depleak, 4.0, 4 , T0 )
         T10 = T10 + epsm10 ;
         T10_dVb *=T0 ;
         T10_dVd *=T0 ;
         T10_dVg *=T0 ;
       ...
       T5 = Fn_Pow( Eeff , model->HSMHV_depmueph0 - 1.0e0 ) ;
       T8 = T5 * Eeff ;
       T8_dT = model->HSMHV_mueph0 * T5 * Eeff_dT ;
       ...
       T1 = 1.0e0 / ( model->HSMHV_depmue0 + model->HSMHV_depmue1 * Rns / 1.0e11 )
         + here->HSMHV_depmphn0 * T8  ;

       T1_dT = - 1.0e0 / ( model->HSMHV_depmue0 + model->HSMHV_depmue1 * Rns / 1.0e11 )
         / ( model->HSMHV_depmue0 + model->HSMHV_depmue1 * Rns / 1.0e11 )
         * model->HSMHV_depmue1 * Rns_dT / 1.0e11
         + here->HSMHV_depmphn0 * T8_dT + depmphn0_dT * T8  ;

       Muun = 1.0e0 / T1 ;
       Muun_dT = - Muun / T1 * T1_dT ;

       T1 = 1.0e0 / ( T1 * T1 ) ;
       T2 = model->HSMHV_depmue0 + model->HSMHV_depmue1 * Rns / 1.0e11 ;
       T2 = 1.0e0 / ( T2 * T2 ) ;
       T3 = here->HSMHV_depmphn1 * T5 ;
       T5 = - 1.0e-11 * model->HSMHV_depmue1 / C_QE * T2 / C_m2cm_p2 ;
       Muun_dVbs = - ( T5 * Qiu_dVbs
                    + Eeff_dVbs * T3 ) * T1 ;
       Muun_dVds = - ( T5 * Qiu_dVds
                    + Eeff_dVds * T3 ) * T1 ;
      ...
      /*  Change to MKS unit */
       ...
       T1 = Muun * Edri / here->HSMHV_depvmax ;
       T1_dVgs = (Muun_dVgs * Edri + Muun * Edri_dVgs) / here->HSMHV_depvmax ;
       T1_dVds = (Muun_dVds * Edri + Muun * Edri_dVds) / here->HSMHV_depvmax ;
       T1_dVbs = (Muun_dVbs * Edri + Muun * Edri_dVbs) / here->HSMHV_depvmax ;
       T1_dT   = (Muun_dT * Edri + Muun * Edri_dT) / here->HSMHV_depvmax
                 - Muun * Edri / here->HSMHV_depvmax / here->HSMHV_depvmax * depVmax_dT ;
       
       T1 = T1 + small ;
       T2 = Fn_Pow(T1,model->HSMHV_depbb) ;
       T2_dVgs = model->HSMHV_depbb * T1_dVgs / T1 * T2 ;
       T2_dVds = model->HSMHV_depbb * T1_dVds / T1 * T2 ;
       T2_dVbs = model->HSMHV_depbb * T1_dVbs / T1 * T2 ;
       T2_dT   = model->HSMHV_depbb * T1_dT / T1 * T2 ;

       T3 = 1.0 + T2 ;
       T4 = Fn_Pow(T3,1.0 / model->HSMHV_depbb) ;
       T4_dVgs = 1.0 / model->HSMHV_depbb * T2_dVgs / T3 * T4 ;
       T4_dVds = 1.0 / model->HSMHV_depbb * T2_dVds / T3 * T4 ;
       T4_dVbs = 1.0 / model->HSMHV_depbb * T2_dVbs / T3 * T4 ;
       T4_dT   = 1.0 / model->HSMHV_depbb * T2_dT / T3 * T4 ;
       ...
       Ids_res = here->HSMHV_weff_nf * ( - Qn_res0) * Mu_res * Edri ;
       Ids_res_dVgs = here->HSMHV_weff_nf * ( - Qn_res0_dVgs * Mu_res * Edri
               - Qn_res0 * Mu_res_dVgs * Edri - Qn_res0 * Mu_res * Edri_dVgs) ;
       Ids_res_dVds = here->HSMHV_weff_nf * ( - Qn_res0_dVds * Mu_res * Edri
               - Qn_res0 * Mu_res_dVds * Edri - Qn_res0 * Mu_res * Edri_dVds) ;
       Ids_res_dVbs = here->HSMHV_weff_nf * ( - Qn_res0_dVbs * Mu_res * Edri
               - Qn_res0 * Mu_res_dVbs * Edri - Qn_res0 * Mu_res * Edri_dVbs) ;
       Ids_res_dT   = here->HSMHV_weff_nf * ( - Qn_res0_dT * Mu_res * Edri
     ...
     //*-----------------//
       ...
       T1 = 1.0e0 / ( model->HSMHV_depmueback0 + model->HSMHV_depmueback1 * Rns / 1.0e11 )
         + here->HSMHV_depmphn0 * T8  ;

       T1_dT = - 1.0e0 / ( model->HSMHV_depmueback0 + model->HSMHV_depmueback1 * Rns / 1.0e11 )
         / ( model->HSMHV_depmueback0 + model->HSMHV_depmueback1 * Rns / 1.0e11 )
         * model->HSMHV_depmueback1 * Rns_dT / 1.0e11
         + here->HSMHV_depmphn0 * T8_dT + depmphn0_dT * T8  ;
       ...
       T1 = 1.0e0 / ( T1 * T1 ) ;
       T2 = model->HSMHV_depmueback0 + model->HSMHV_depmueback1 * Rns / 1.0e11 ;
       T2 = 1.0e0 / ( T2 * T2 ) ;
       T3 = here->HSMHV_depmphn1 * T5 ;
       T5 = - 1.0e-11 * model->HSMHV_depmueback1 / C_QE * T2 / C_m2cm_p2 ;
       Muun_dVbs = - ( T5 * Qiu_dVbs 
                    + Eeff_dVbs * T3 ) * T1 ;
       Muun_dVds = - ( T5 * Qiu_dVds 
                    + Eeff_dVds * T3 ) * T1 ;
      ...
      /*  Change to MKS unit */
       ...
       Ids_bac = here->HSMHV_weff_nf * ( - Qn_bac0) * Mu_bac * Edri ;
       Ids_bac_dVgs = here->HSMHV_weff_nf * ( - Qn_bac0_dVgs * Mu_bac * Edri
               - Qn_bac0 * Mu_bac_dVgs * Edri - Qn_bac0 * Mu_bac * Edri_dVgs) ;
       Ids_bac_dVds = here->HSMHV_weff_nf * ( - Qn_bac0_dVds * Mu_bac * Edri
               - Qn_bac0 * Mu_bac_dVds * Edri - Qn_bac0 * Mu_bac * Edri_dVds) ;
       Ids_bac_dVbs = here->HSMHV_weff_nf * ( - Qn_bac0_dVbs * Mu_bac * Edri
               - Qn_bac0 * Mu_bac_dVbs * Edri - Qn_bac0 * Mu_bac * Edri_dVbs) ;
       Ids_bac_dT   = here->HSMHV_weff_nf * ( - Qn_bac0_dT * Mu_bac * Edri
      ...
      *-----------------*/
       ...
       T1 = - betaWL / Lch ;
       betaWL_dT = here->HSMHV_weff_nf * beta_inv_dT / Lch  ;
       ...
       *-----------------*/
       if( model->HSMHV_ptl != 0 ){
         ...
         T0 = beta * here->HSMHV_ptl0 ;
         T0_dT = beta_dT * here->HSMHV_ptl0 ;
         T3 = Cox * T0 ;
         T3_dVb = Cox_dVb * T0 ;
         T3_dVd = Cox_dVd * T0 ;
         T3_dVg = Cox_dVg * T0 ;
         T3_dT = Cox_dT * T0 + Cox * T0_dT ;
         T0 = pow( T2 , model->HSMHV_ptp ) ;
         T9     = T3 * T0 ;
         T9_dVb = T3 * model->HSMHV_ptp * T0 / T2 * T2_dVb + T3_dVb * T0 ;
         T9_dVd = T3 * model->HSMHV_ptp * T0 / T2 * T2_dVd + T3_dVd * T0 ;
         T9_dVg = T3 * model->HSMHV_ptp * T0 / T2 * T2_dVg + T3_dVg * T0 ;
         T9_dT = T3 * model->HSMHV_ptp * T0 / T2 * T2_dT + T3_dT * T0 ;


         T4 = 1.0 + Vdsz * model->HSMHV_pt2 ;
         T4_dVb = Vdsz_dVbs * model->HSMHV_pt2 ;
         T4_dVd = Vdsz_dVds * model->HSMHV_pt2 ;
         T4_dVg = 0.0 ;
         T4_dT = Vdsz_dT * model->HSMHV_pt2 ;
       ...
       if( model->HSMHV_gdl != 0 ){
         T1 = beta * here->HSMHV_gdl0 ;
         T1_dT = beta_dT * here->HSMHV_gdl0 ;
         T2 = Cox * T1 ;
         T2_dVb = Cox_dVb * T1 ;
         T2_dVd = Cox_dVd * T1 ;
         T2_dVg = Cox_dVg * T1 ;
