      ...
      */
{
  ...
  for ( ;model != NULL ;model = HSMHVnextModel(model)) {
    ...
    if ( !model->HSMHV_version_Given) {
      ...
      if (strncmp(model->HSMHV_version,"2.20", 4) == 0 ) {
        printf("HiSIM_HV(%s): 2.20 is selected for VERSION. (default) \n",model->HSMHVmodName);
        model->HSMHV_subversion = 2 ;
      } else if (strncmp(model->HSMHV_version,"2.2", 3) == 0 ) {
        printf("HiSIM_HV(%s): 2.20 is selected for VERSION. (default) \n",model->HSMHVmodName);
        model->HSMHV_subversion = 2 ;
      } else {
...
//                                                                        here->HSMHV_lover  = here->HSMHV_lovers ;
	...
	if( here->HSMHVdNodePrime <= 0) {
        error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname, "drain");
	...
	here->HSMHVdNodePrime = here->HSMHVdNode;
      ...
      if ( (model->HSMHV_corsrd == 1 || model->HSMHV_corsrd == 3 || model->HSMHV_cordrift == 1)
       ...
       if( here->HSMHVsNodePrime == 0) {
        error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname, "source");
	if (error) return(error);
	...
	error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname, "gate");
	if (error) return(error);
	...
	if (here->HSMHVdbNode == 0) {
	  error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname, "dbody");
	  if (error) return(error);
	  here->HSMHVdbNode = tmp->number;
	}
	if (here->HSMHVbNodePrime == 0) {
	  error = Shim::CKTmkVolt(ckt, &tmp,here->HSMHVname, "body");
	  if (error) return(error);
	  here->HSMHVbNodePrime = tmp->number;
	}
	if (here->HSMHVsbNode == 0) {
	  error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname,"sbody");
	  if (error) return(error);
	...
	error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname,"temp");
	if(error) return(error);
	here->HSMHVtempNode = tmp->number;
      ...
      if ( model->HSMHV_conqs ){
	error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname,"qi_nqs");
	if(error) return(error);
	here->HSMHVqiNode = tmp->number;
	error = Shim::CKTmkVolt(ckt, &tmp, here->HSMHVname,"qb_nqs");
	if(error) return(error);
            ...
            model = HSMHVnextModel(model))
    {
                ...
                here=HSMHVnextInstance(here))
        {
            if (here->HSMHVtempNode > 0 &&
                ...
                Shim::CKTdltNNum(ckt, here->HSMHVtempNode);
            here->HSMHVtempNode = 0;

            here->HSMHVsubNode = 0;

            if (here->HSMHVqbNode > 0)
                Shim::CKTdltNNum(ckt, here->HSMHVqbNode);
            here->HSMHVqbNode = 0;

            if (here->HSMHVqiNode > 0)
                Shim::CKTdltNNum(ckt, here->HSMHVqiNode);
            here->HSMHVqiNode = 0;

            if (here->HSMHVsbNode > 0
                    && here->HSMHVsbNode != here->HSMHVbNode)
                Shim::CKTdltNNum(ckt, here->HSMHVsbNode);
            here->HSMHVsbNode = 0;

            if (here->HSMHVbNodePrime > 0
                    && here->HSMHVbNodePrime != here->HSMHVbNode)
                Shim::CKTdltNNum(ckt, here->HSMHVbNodePrime);
            here->HSMHVbNodePrime = 0;

            if (here->HSMHVdbNode > 0
                    && here->HSMHVdbNode != here->HSMHVbNode)
                Shim::CKTdltNNum(ckt, here->HSMHVdbNode);
            here->HSMHVdbNode = 0;

            if (here->HSMHVgNodePrime > 0
                    && here->HSMHVgNodePrime != here->HSMHVgNode)
                Shim::CKTdltNNum(ckt, here->HSMHVgNodePrime);
            here->HSMHVgNodePrime = 0;

            if (here->HSMHVsNodePrime > 0
                    && here->HSMHVsNodePrime != here->HSMHVsNode)
                Shim::CKTdltNNum(ckt, here->HSMHVsNodePrime);
            here->HSMHVsNodePrime = 0;

            if (here->HSMHVdNodePrime > 0
                    && here->HSMHVdNodePrime != here->HSMHVdNode)
                Shim::CKTdltNNum(ckt, here->HSMHVdNodePrime);
