...
*/
      ...
      *(ckt->CKTrhs + here->HSMHVsbNode)     -= model->HSMHV_type * cur_sb;
      ...
      /*drain*/
      mat.add(here->HSMHVDdPtr, ydc_d[dNode]         + ag0*ydyn_d[dNode]);
      mat.add(here->HSMHVDdpPtr, ydc_d[dNodePrime]    + ag0*ydyn_d[dNodePrime]);
      mat.add(here->HSMHVDgpPtr, ydc_d[gNodePrime]    + ag0*ydyn_d[gNodePrime]);
      mat.add(here->HSMHVDsPtr, ydc_d[sNode]         + ag0*ydyn_d[sNode]);
      mat.add(here->HSMHVDspPtr, ydc_d[sNodePrime]    + ag0*ydyn_d[sNodePrime]);
      mat.add(here->HSMHVDbpPtr, ydc_d[bNodePrime]    + ag0*ydyn_d[bNodePrime]);
      mat.add(here->HSMHVDdbPtr, ydc_d[dbNode]        + ag0*ydyn_d[dbNode]);
      if (flg_subNode > 0) {
        mat.add(here->HSMHVDsubPtr, ydc_d[subNode]);
      }
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVDtempPtr, model->HSMHV_type * (ydc_d[tempNode] + ag0*ydyn_d[tempNode]));
      }

      /*drain prime*/
      mat.add(here->HSMHVDPdPtr, ydc_dP[dNode]      + ag0*ydyn_dP[dNode]);
      mat.add(here->HSMHVDPdpPtr, ydc_dP[dNodePrime] + ag0*ydyn_dP[dNodePrime]);
      mat.add(here->HSMHVDPgpPtr, ydc_dP[gNodePrime] + ag0*ydyn_dP[gNodePrime]);
      mat.add(here->HSMHVDPsPtr, ydc_dP[sNode]      + ag0*ydyn_dP[sNode]);
      mat.add(here->HSMHVDPspPtr, ydc_dP[sNodePrime] + ag0*ydyn_dP[sNodePrime]);
      mat.add(here->HSMHVDPbpPtr, ydc_dP[bNodePrime] + ag0*ydyn_dP[bNodePrime]);
      if (flg_subNode > 0) {
        mat.add(here->HSMHVDPsubPtr, ydc_dP[subNode]);
      }
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVDPtempPtr, model->HSMHV_type * (ydc_dP[tempNode] + ag0*ydyn_dP[tempNode]));
      }
      if (flg_nqs) {
        mat.add(here->HSMHVDPqiPtr, model->HSMHV_type * (ydc_dP[qiNode] + ag0*ydyn_dP[qiNode]));
      }

      /*gate*/
      mat.add(here->HSMHVGgPtr, ydc_g[gNode]       + ag0*ydyn_g[gNode]);
      mat.add(here->HSMHVGgpPtr, ydc_g[gNodePrime]  + ag0*ydyn_g[gNodePrime]);

      /*gate prime*/
      mat.add(here->HSMHVGPdPtr, ydc_gP[dNode]      + ag0*ydyn_gP[dNode]);
      mat.add(here->HSMHVGPdpPtr, ydc_gP[dNodePrime] + ag0*ydyn_gP[dNodePrime]);
      mat.add(here->HSMHVGPgPtr, ydc_gP[gNode]      + ag0*ydyn_gP[gNode]);
      mat.add(here->HSMHVGPgpPtr, ydc_gP[gNodePrime] + ag0*ydyn_gP[gNodePrime]);
      mat.add(here->HSMHVGPsPtr, ydc_gP[sNode]      + ag0*ydyn_gP[sNode]);
      mat.add(here->HSMHVGPspPtr, ydc_gP[sNodePrime] + ag0*ydyn_gP[sNodePrime]);
      mat.add(here->HSMHVGPbpPtr, ydc_gP[bNodePrime] + ag0*ydyn_gP[bNodePrime]);
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVGPtempPtr, model->HSMHV_type * (ydc_gP[tempNode] + ag0*ydyn_gP[tempNode]));
      }
      if (flg_nqs) {
        mat.add(here->HSMHVGPqiPtr, model->HSMHV_type * (ydc_gP[qiNode] + ag0*ydyn_gP[qiNode]));
        mat.add(here->HSMHVGPqbPtr, model->HSMHV_type * (ydc_gP[qbNode] + ag0*ydyn_gP[qbNode]));
      }

      /*source*/
      mat.add(here->HSMHVSdPtr, ydc_s[dNode]         + ag0*ydyn_s[dNode]);
      mat.add(here->HSMHVSsPtr, ydc_s[sNode]         + ag0*ydyn_s[sNode]);
      mat.add(here->HSMHVSdpPtr, ydc_s[dNodePrime]    + ag0*ydyn_s[dNodePrime]);
      mat.add(here->HSMHVSgpPtr, ydc_s[gNodePrime]    + ag0*ydyn_s[gNodePrime]);
      mat.add(here->HSMHVSspPtr, ydc_s[sNodePrime]    + ag0*ydyn_s[sNodePrime]);
      mat.add(here->HSMHVSbpPtr, ydc_s[bNodePrime]    + ag0*ydyn_s[bNodePrime]);
      mat.add(here->HSMHVSsbPtr, ydc_s[sbNode]        + ag0*ydyn_s[sbNode]);
      if (flg_subNode > 0) {
        mat.add(here->HSMHVSsubPtr, ydc_s[subNode]);
      }
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVStempPtr, model->HSMHV_type * (ydc_s[tempNode]+ ag0*ydyn_s[tempNode]));
      }

      /*source prime*/
      mat.add(here->HSMHVSPdPtr, ydc_sP[dNode]      + ag0*ydyn_sP[dNode]);
      mat.add(here->HSMHVSPdpPtr, ydc_sP[dNodePrime] + ag0*ydyn_sP[dNodePrime]);
      mat.add(here->HSMHVSPgpPtr, ydc_sP[gNodePrime] + ag0*ydyn_sP[gNodePrime]);
      mat.add(here->HSMHVSPsPtr, ydc_sP[sNode]      + ag0*ydyn_sP[sNode]);
      mat.add(here->HSMHVSPspPtr, ydc_sP[sNodePrime] + ag0*ydyn_sP[sNodePrime]);
      mat.add(here->HSMHVSPbpPtr, ydc_sP[bNodePrime] + ag0*ydyn_sP[bNodePrime]);
      if (flg_subNode > 0) {
        mat.add(here->HSMHVSPsubPtr, ydc_sP[subNode]);
      }
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVSPtempPtr, model->HSMHV_type * (ydc_sP[tempNode] + ag0*ydyn_sP[tempNode]));
      }
      if (flg_nqs) {
        mat.add(here->HSMHVSPqiPtr, model->HSMHV_type * (ydc_sP[qiNode] + ag0*ydyn_sP[qiNode]));
      }

      /*bulk prime*/
      mat.add(here->HSMHVBPdPtr, ydc_bP[dNode]      + ag0*ydyn_bP[dNode]);
      mat.add(here->HSMHVBPdpPtr, ydc_bP[dNodePrime] + ag0*ydyn_bP[dNodePrime]);
      mat.add(here->HSMHVBPgpPtr, ydc_bP[gNodePrime] + ag0*ydyn_bP[gNodePrime]);
      mat.add(here->HSMHVBPspPtr, ydc_bP[sNodePrime] + ag0*ydyn_bP[sNodePrime]);
      mat.add(here->HSMHVBPsPtr, ydc_bP[sNode]      + ag0*ydyn_bP[sNode]);
      mat.add(here->HSMHVBPbpPtr, ydc_bP[bNodePrime] + ag0*ydyn_bP[bNodePrime]);
      mat.add(here->HSMHVBPbPtr, ydc_bP[bNode]      + ag0*ydyn_bP[bNode]);
      mat.add(here->HSMHVBPdbPtr, ydc_bP[dbNode]     + ag0*ydyn_bP[dbNode]);
      mat.add(here->HSMHVBPsbPtr, ydc_bP[sbNode]     + ag0*ydyn_bP[sbNode]);
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVBPtempPtr, model->HSMHV_type * (ydc_bP[tempNode] + ag0*ydyn_bP[tempNode]));
      }
      if (flg_nqs) {
        mat.add(here->HSMHVBPqbPtr, model->HSMHV_type * (ydc_bP[qbNode] + ag0*ydyn_bP[qbNode]));
      }

      /*bulk*/
      mat.add(here->HSMHVBbpPtr, ydc_b[bNodePrime]   + ag0*ydyn_b[bNodePrime]);
      mat.add(here->HSMHVBbPtr, ydc_b[bNode]        + ag0*ydyn_b[bNode]);

      /*drain bulk*/
      mat.add(here->HSMHVDBdPtr, ydc_db[dNode]      + ag0*ydyn_db[dNode]);
      mat.add(here->HSMHVDBbpPtr, ydc_db[bNodePrime] + ag0*ydyn_db[bNodePrime]);
      mat.add(here->HSMHVDBdbPtr, ydc_db[dbNode]     + ag0*ydyn_db[dbNode]);
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVDBtempPtr, model->HSMHV_type * (ydc_db[tempNode] + ag0*ydyn_db[tempNode]));
      }

      /*source bulk*/
      mat.add(here->HSMHVSBsPtr, ydc_sb[sNode]      + ag0*ydyn_sb[sNode]);
      mat.add(here->HSMHVSBbpPtr, ydc_sb[bNodePrime] + ag0*ydyn_sb[bNodePrime]);
      mat.add(here->HSMHVSBsbPtr, ydc_sb[sbNode]     + ag0*ydyn_sb[sbNode]);
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVSBtempPtr, model->HSMHV_type * (ydc_sb[tempNode] + ag0*ydyn_sb[tempNode]));
      }

      /*temp*/
      if( here->HSMHV_coselfheat > 0) {
        /* temp entries in matrix dependent on model type */
        mat.add(here->HSMHVTempdPtr, model->HSMHV_type * (ydc_t[dNode]      + ag0*ydyn_t[dNode]     ));
        mat.add(here->HSMHVTempdpPtr, model->HSMHV_type * (ydc_t[dNodePrime] + ag0*ydyn_t[dNodePrime]));
        mat.add(here->HSMHVTempgpPtr, model->HSMHV_type * (ydc_t[gNodePrime] + ag0*ydyn_t[gNodePrime]));
        mat.add(here->HSMHVTempsPtr, model->HSMHV_type * (ydc_t[sNode]      + ag0*ydyn_t[sNode]     ));
        mat.add(here->HSMHVTempspPtr, model->HSMHV_type * (ydc_t[sNodePrime] + ag0*ydyn_t[sNodePrime]));
        mat.add(here->HSMHVTempbpPtr, model->HSMHV_type * (ydc_t[bNodePrime] + ag0*ydyn_t[bNodePrime]));
        /* no type factor at main diagonal temp entry! */
        mat.add(here->HSMHVTemptempPtr, ydc_t[tempNode] + ag0*ydyn_t[tempNode]);
      }

      /* additional entries for flat nqs handling */
      if ( flg_nqs ) {
        /*qi*/
        mat.add(here->HSMHVQIdpPtr, model->HSMHV_type * (ydc_qi[dNodePrime] + ag0*ydyn_qi[dNodePrime]));
        mat.add(here->HSMHVQIgpPtr, model->HSMHV_type * (ydc_qi[gNodePrime] + ag0*ydyn_qi[gNodePrime]));
        mat.add(here->HSMHVQIspPtr, model->HSMHV_type * (ydc_qi[sNodePrime] + ag0*ydyn_qi[sNodePrime]));
        mat.add(here->HSMHVQIbpPtr, model->HSMHV_type * (ydc_qi[bNodePrime] + ag0*ydyn_qi[bNodePrime]));
        mat.add(here->HSMHVQIqiPtr, (ydc_qi[qiNode] + ag0*ydyn_qi[qiNode]));
        if ( here->HSMHV_coselfheat > 0 ) { /* self heating */
          mat.add(here->HSMHVQItempPtr, (ydc_qi[tempNode] + ag0*ydyn_qi[tempNode]));
        }

        /*qb*/
        mat.add(here->HSMHVQBdpPtr, model->HSMHV_type * (ydc_qb[dNodePrime] + ag0*ydyn_qb[dNodePrime]));
        mat.add(here->HSMHVQBgpPtr, model->HSMHV_type * (ydc_qb[gNodePrime] + ag0*ydyn_qb[gNodePrime]));
        mat.add(here->HSMHVQBspPtr, model->HSMHV_type * (ydc_qb[sNodePrime] + ag0*ydyn_qb[sNodePrime]));
        mat.add(here->HSMHVQBbpPtr, model->HSMHV_type * (ydc_qb[bNodePrime] + ag0*ydyn_qb[bNodePrime]));
        mat.add(here->HSMHVQBqbPtr, (ydc_qb[qbNode] + ag0*ydyn_qb[qbNode]));
        if ( here->HSMHV_coselfheat > 0 ) { /* self heating */
          mat.add(here->HSMHVQBtempPtr, (ydc_qb[tempNode] + ag0*ydyn_qb[tempNode]));
        }
      }
...
line1000:
      ...
      if (ckt->CKTnoncon != noncon_old) {
        ckt->CKTtroubleElt = (HSMHVInstance *) here;
      }
