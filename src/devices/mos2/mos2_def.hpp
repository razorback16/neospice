...
struct MOS2Instance {
    MOS2Model *MOS2modPtr; /* backpointer to model */
    MOS2Instance *MOS2nextInstance;  /* pointer to next instance of
                                              *current model*/
    const char *MOS2name; /* pointer to character string naming this instance */
    ...
    neospice::MatrixOffset MOS2DdPtr{-1};      /* pointer to sparse matrix element at
                                     * (Drain node,drain node) */
    neospice::MatrixOffset MOS2GgPtr{-1};      /* pointer to sparse matrix element at
                                     * (gate node,gate node) */
    neospice::MatrixOffset MOS2SsPtr{-1};      /* pointer to sparse matrix element at
                                     * (source node,source node) */
    neospice::MatrixOffset MOS2BbPtr{-1};      /* pointer to sparse matrix element at
                                     * (bulk node,bulk node) */
    neospice::MatrixOffset MOS2DPdpPtr{-1};    /* pointer to sparse matrix element at
                                     * (drain prime node,drain prime node) */
    neospice::MatrixOffset MOS2SPspPtr{-1};    /* pointer to sparse matrix element at
                                     * (source prime node,source prime node) */
    neospice::MatrixOffset MOS2DdpPtr{-1};     /* pointer to sparse matrix element at
                                     * (drain node,drain prime node) */
    neospice::MatrixOffset MOS2GbPtr{-1};      /* pointer to sparse matrix element at
                                     * (gate node,bulk node) */
    neospice::MatrixOffset MOS2GdpPtr{-1};     /* pointer to sparse matrix element at
                                     * (gate node,drain prime node) */
    neospice::MatrixOffset MOS2GspPtr{-1};     /* pointer to sparse matrix element at
                                     * (gate node,source prime node) */
    neospice::MatrixOffset MOS2SspPtr{-1};     /* pointer to sparse matrix element at
                                     * (source node,source prime node) */
    neospice::MatrixOffset MOS2BdpPtr{-1};     /* pointer to sparse matrix element at
                                     * (bulk node,drain prime node) */
    neospice::MatrixOffset MOS2BspPtr{-1};     /* pointer to sparse matrix element at
                                     * (bulk node,source prime node) */
    neospice::MatrixOffset MOS2DPspPtr{-1};    /* pointer to sparse matrix element at
                                     * (drain prime node,source prime node) */
    neospice::MatrixOffset MOS2DPdPtr{-1};     /* pointer to sparse matrix element at
                                     * (drain prime node,drain node) */
    neospice::MatrixOffset MOS2BgPtr{-1};      /* pointer to sparse matrix element at
                                     * (bulk node,gate node) */
    neospice::MatrixOffset MOS2DPgPtr{-1};     /* pointer to sparse matrix element at
                                     * (drain prime node,gate node) */

    neospice::MatrixOffset MOS2SPgPtr{-1};     /* pointer to sparse matrix element at
                                     * (source prime node,gate node) */
    neospice::MatrixOffset MOS2SPsPtr{-1};     /* pointer to sparse matrix element at
                                     * (source prime node,source node) */
    neospice::MatrixOffset MOS2DPbPtr{-1};     /* pointer to sparse matrix element at
                                     * (drain prime node,bulk node) */
    neospice::MatrixOffset MOS2SPbPtr{-1};     /* pointer to sparse matrix element at
                                     * (source prime node,bulk node) */
    neospice::MatrixOffset MOS2SPdpPtr{-1};    /* pointer to sparse matrix element at
...
struct MOS2Model {       /* model structure for a resistor */
    int MOS2modType;    /* type index to this device type */
    MOS2Model *MOS2nextModel;    /* pointer to next possible model
                                          *in linked list */
    MOS2Instance * MOS2instances; /* pointer to list of instances
                                   * that have this model */
    const char *MOS2modName;       /* pointer to character string naming this model */

    /* --- end of generic struct MOS2Model --- */

/* Compatibility shims: the original ngspice macros walked the GENinstance/
 * GENmodel linked-list indirection.  Here the fields are flattened directly
 * into the instance/model structs, so map the function-like accessors to the
 * plain field accesses used by the translated load/setup/temp sources. */
#define MOS2nextModel(m)    ((m)->MOS2nextModel)
#define MOS2instances(m)    ((m)->MOS2instances)
#define MOS2nextInstance(i) ((i)->MOS2nextInstance)
#define MOS2modPtr(i)       ((i)->MOS2modPtr)
...
extern Shim::IfParm MOS2pTable[];
extern int MOS2pTSize;
extern Shim::IfParm MOS2mPTable[];
extern int MOS2mPTSize;
int MOS2mParam(int param, Shim::IfValue *value, MOS2Model *model);
int MOS2param(int param, Shim::IfValue *value, MOS2Instance *inst, Shim::IfValue *select);
