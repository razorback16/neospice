...
struct MOS2ModelCard {
    mos2::MOS2Model ucb{};   // aggregate UCB model fields

    MOS2ModelCard() = default;
    ~MOS2ModelCard();

    // Non-copyable / non-movable: UCB model is threaded with raw
    // pointers; copying would alias them.
    MOS2ModelCard(const MOS2ModelCard&)            = delete;
    MOS2ModelCard& operator=(const MOS2ModelCard&) = delete;
};
...
public:
    ...
    static std::unique_ptr<MOS2Device> make(
...
private:
    explicit MOS2Device(std::string name) : Device(std::move(name)) {}

    mutable mos2::MOS2Instance inst_{};
    mos2::MOS2Model* model_ = nullptr;
