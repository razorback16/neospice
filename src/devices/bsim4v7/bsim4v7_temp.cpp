 ...
 *
 * Mechanical translation to C++ by neospice Z-port Task 10.
 * Original: third_party/bsim4_4.7.0/code/b4temp.c
 * (per-instance temperature-dependent precompute + binning interpolation).
 **********/
...
/* SPICE3 const.h constants consumed by BSIM4temp. CONSTvt0 is the
 * thermal voltage at T = 300.15 K (k*T/q); CONSTroot2 is sqrt(2). */
#ifndef CONSTvt0
...
int
BSIM4temp(BSIM4v7Model *model, Shim::Ckt *ckt)
{
		...
		{ 
			...
			niter++;
              ...
              {   const char *namarray[2];
                  namarray[0] = model->BSIM4modName;
                  namarray[1] = here->BSIM4name;
                  Shim::report_error(Shim::ERR_FATAL, "Fatal error(s) detected during BSIM4.6.0 parameter checking for %s in model %s", namarray[0], namarray[1]);
                  return Shim::E_BADPARM;
              }
         } /* End instance */
    }
