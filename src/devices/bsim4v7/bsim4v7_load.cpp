...
// Hand-fix (Phase 1b Task 6): constants / helpers pulled in from UCB headers
// that the translator does not know about. Kept local so they don't leak.
#ifndef CONSTvt0
...
int
BSIM4load(BSIM4v7Model * inModel, Shim::Ckt * ckt) {
register BSIM4v7Model *model = inModel;
register BSIM4v7Instance *here;
// Hand-fix (Phase 1b Task 6): bind the matrix through the Ckt shim so that
...
line900:
	  ...
	  {   Gm = -here->BSIM4gm;
	   ...
	   geltd = here->BSIM4grgeltd;
           ...
           if (here->BSIM4rgateMod == 1)
           {   mat.add(here->BSIM4GEgePtr, geltd);
               mat.add(here->BSIM4GPgePtr, -(geltd));
               mat.add(here->BSIM4GEgpPtr, -(geltd));
  	       mat.add(here->BSIM4GPgpPtr, gcggb + geltd - ggtg + gIgtotg);
  	       mat.add(here->BSIM4GPdpPtr, gcgdb - ggtd + gIgtotd);
   	       mat.add(here->BSIM4GPspPtr, gcgsb - ggts + gIgtots);
	       mat.add(here->BSIM4GPbpPtr, gcgbb - ggtb + gIgtotb);
           } /* WDLiu: gcrg already subtracted from all gcrgg below */
           else if (here->BSIM4rgateMod == 2)	
	   {   mat.add(here->BSIM4GEgePtr, gcrg);
               mat.add(here->BSIM4GEgpPtr, gcrgg);
               mat.add(here->BSIM4GEdpPtr, gcrgd);
               mat.add(here->BSIM4GEspPtr, gcrgs);
	       mat.add(here->BSIM4GEbpPtr, gcrgb);	

               mat.add(here->BSIM4GPgePtr, -(gcrg));
  	       mat.add(here->BSIM4GPgpPtr, gcggb  - gcrgg - ggtg + gIgtotg);
	       mat.add(here->BSIM4GPdpPtr, gcgdb - gcrgd - ggtd + gIgtotd);
	       mat.add(here->BSIM4GPspPtr, gcgsb - gcrgs - ggts + gIgtots);
  	       mat.add(here->BSIM4GPbpPtr, gcgbb - gcrgb - ggtb + gIgtotb);
	   }
	   else if (here->BSIM4rgateMod == 3)
	   {   mat.add(here->BSIM4GEgePtr, geltd);
               mat.add(here->BSIM4GEgmPtr, -(geltd));
               mat.add(here->BSIM4GMgePtr, -(geltd));
               mat.add(here->BSIM4GMgmPtr, geltd + gcrg + gcgmgmb);

               mat.add(here->BSIM4GMdpPtr, gcrgd + gcgmdb);
               mat.add(here->BSIM4GMgpPtr, gcrgg);
               mat.add(here->BSIM4GMspPtr, gcrgs + gcgmsb);
               mat.add(here->BSIM4GMbpPtr, gcrgb + gcgmbb);

               mat.add(here->BSIM4DPgmPtr, gcdgmb);
               mat.add(here->BSIM4GPgmPtr, -(gcrg));
               mat.add(here->BSIM4SPgmPtr, gcsgmb);
               mat.add(here->BSIM4BPgmPtr, gcbgmb);
	   ...
	   {   mat.add(here->BSIM4GPgpPtr, gcggb - ggtg + gIgtotg);
  	       mat.add(here->BSIM4GPdpPtr, gcgdb - ggtd + gIgtotd);
               mat.add(here->BSIM4GPspPtr, gcgsb - ggts + gIgtots);
	       mat.add(here->BSIM4GPbpPtr, gcgbb - ggtb + gIgtotb);
	   }
	   ...
	   if (model->BSIM4rdsMod)
	   {   mat.add(here->BSIM4DgpPtr, gdtotg);
	       mat.add(here->BSIM4DspPtr, gdtots);
               mat.add(here->BSIM4DbpPtr, gdtotb);
               mat.add(here->BSIM4SdpPtr, gstotd);
               mat.add(here->BSIM4SgpPtr, gstotg);
               mat.add(here->BSIM4SbpPtr, gstotb);
	   }

           mat.add(here->BSIM4DPdpPtr, gdpr + here->BSIM4gds + here->BSIM4gbd + T1 * ddxpart_dVd
                                   - gdtotd + RevSum + gcddb + gbdpdp + dxpart * ggtd - gIdtotd);
           mat.add(here->BSIM4DPdPtr, -(gdpr + gdtot));
           mat.add(here->BSIM4DPgpPtr, Gm + gcdgb - gdtotg + gbdpg - gIdtotg
				   + dxpart * ggtg + T1 * ddxpart_dVg);
           mat.add(here->BSIM4DPspPtr, -(here->BSIM4gds + gdtots - dxpart * ggts + gIdtots
				   - T1 * ddxpart_dVs + FwdSum - gcdsb - gbdpsp));
           mat.add(here->BSIM4DPbpPtr, -(gjbd + gdtotb - Gmbs - gcdbb - gbdpb + gIdtotb
				   - T1 * ddxpart_dVb - dxpart * ggtb));

           mat.add(here->BSIM4DdpPtr, -(gdpr - gdtotd));
           mat.add(here->BSIM4DdPtr, gdpr + gdtot);

           mat.add(here->BSIM4SPdpPtr, -(here->BSIM4gds + gstotd + RevSum - gcsdb - gbspdp
				   - T1 * dsxpart_dVd - sxpart * ggtd + gIstotd));
           mat.add(here->BSIM4SPgpPtr, gcsgb - Gm - gstotg + gbspg + sxpart * ggtg
				   + T1 * dsxpart_dVg - gIstotg);
           mat.add(here->BSIM4SPspPtr, gspr + here->BSIM4gds + here->BSIM4gbs + T1 * dsxpart_dVs
                                   - gstots + FwdSum + gcssb + gbspsp + sxpart * ggts - gIstots);
           mat.add(here->BSIM4SPsPtr, -(gspr + gstot));
           mat.add(here->BSIM4SPbpPtr, -(gjbs + gstotb + Gmbs - gcsbb - gbspb - sxpart * ggtb
				   - T1 * dsxpart_dVb + gIstotb));

           mat.add(here->BSIM4SspPtr, -(gspr - gstots));
           mat.add(here->BSIM4SsPtr, gspr + gstot);

           mat.add(here->BSIM4BPdpPtr, gcbdb - gjbd + gbbdp - gIbtotd);
           mat.add(here->BSIM4BPgpPtr, gcbgb - here->BSIM4gbgs - gIbtotg);
           mat.add(here->BSIM4BPspPtr, gcbsb - gjbs + gbbsp - gIbtots);
           mat.add(here->BSIM4BPbpPtr, gjbd + gjbs + gcbbb - here->BSIM4gbbs
				   - gIbtotb);
           ...
           /* stamp gidl */
           mat.add(here->BSIM4DPdpPtr, ggidld);
           mat.add(here->BSIM4DPgpPtr, ggidlg);
           mat.add(here->BSIM4DPspPtr, -((ggidlg + ggidld + ggidlb)));
           mat.add(here->BSIM4DPbpPtr, ggidlb);
           mat.add(here->BSIM4BPdpPtr, -(ggidld));
           mat.add(here->BSIM4BPgpPtr, -(ggidlg));
           mat.add(here->BSIM4BPspPtr, (ggidlg + ggidld + ggidlb));
           mat.add(here->BSIM4BPbpPtr, -(ggidlb));
            /* stamp gisl */
           mat.add(here->BSIM4SPdpPtr, -((ggisls + ggislg + ggislb)));
           mat.add(here->BSIM4SPgpPtr, ggislg);
           mat.add(here->BSIM4SPspPtr, ggisls);
           mat.add(here->BSIM4SPbpPtr, ggislb);
           mat.add(here->BSIM4BPdpPtr, (ggislg + ggisls + ggislb));
           mat.add(here->BSIM4BPgpPtr, -(ggislg));
           mat.add(here->BSIM4BPspPtr, -(ggisls));
           mat.add(here->BSIM4BPbpPtr, -(ggislb));


           if (here->BSIM4rbodyMod)
           {   mat.add(here->BSIM4DPdbPtr, gcdbdb - here->BSIM4gbd);
               mat.add(here->BSIM4SPsbPtr, -(here->BSIM4gbs - gcsbsb));

               mat.add(here->BSIM4DBdpPtr, gcdbdb - here->BSIM4gbd);
               mat.add(here->BSIM4DBdbPtr, here->BSIM4gbd - gcdbdb 
                                       + here->BSIM4grbpd + here->BSIM4grbdb);
               mat.add(here->BSIM4DBbpPtr, -(here->BSIM4grbpd));
               mat.add(here->BSIM4DBbPtr, -(here->BSIM4grbdb));

               mat.add(here->BSIM4BPdbPtr, -(here->BSIM4grbpd));
               mat.add(here->BSIM4BPbPtr, -(here->BSIM4grbpb));
               mat.add(here->BSIM4BPsbPtr, -(here->BSIM4grbps));
               mat.add(here->BSIM4BPbpPtr, here->BSIM4grbpd + here->BSIM4grbps 
                                       + here->BSIM4grbpb);
	       /* WDLiu: (gcbbb - here->BSIM4gbbs) already added to BPbpPtr */	

               mat.add(here->BSIM4SBspPtr, gcsbsb - here->BSIM4gbs);
               mat.add(here->BSIM4SBbpPtr, -(here->BSIM4grbps));
               mat.add(here->BSIM4SBbPtr, -(here->BSIM4grbsb));
               mat.add(here->BSIM4SBsbPtr, here->BSIM4gbs - gcsbsb 
                                       + here->BSIM4grbps + here->BSIM4grbsb);

               mat.add(here->BSIM4BdbPtr, -(here->BSIM4grbdb));
               mat.add(here->BSIM4BbpPtr, -(here->BSIM4grbpb));
               mat.add(here->BSIM4BsbPtr, -(here->BSIM4grbsb));
               mat.add(here->BSIM4BbPtr, here->BSIM4grbsb + here->BSIM4grbdb
                                     + here->BSIM4grbpb);
           }

           if (here->BSIM4trnqsMod)
           {   mat.add(here->BSIM4QqPtr, gqdef + here->BSIM4gtau);
               mat.add(here->BSIM4QgpPtr, ggtg - gcqgb);
               mat.add(here->BSIM4QdpPtr, ggtd - gcqdb);
               mat.add(here->BSIM4QspPtr, ggts - gcqsb);
               mat.add(here->BSIM4QbpPtr, ggtb - gcqbb);

               mat.add(here->BSIM4DPqPtr, dxpart * here->BSIM4gtau);
               mat.add(here->BSIM4SPqPtr, sxpart * here->BSIM4gtau);
               mat.add(here->BSIM4GPqPtr, -(here->BSIM4gtau));
           }
