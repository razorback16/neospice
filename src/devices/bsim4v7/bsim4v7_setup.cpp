...
double Rtot, DMCGeff, DMCIeff, DMDGeff;
    ...
    /* Phase-1a Z-port: UCB's noise-analysis job-list lookup is stripped
     * (ft_curckt / TSKtask are front-end state not yet wired up). Default
     * noiseAnalGiven = 0 matches the common non-noise-analysis path. */
	...
	{   /* allocate a chunk of the state vector */
	    ...
	    if (!here->BSIM4scGiven)
            ...
            {
                here->BSIM4dNodePrime = ckt->add_internal_node("drain");
            }
            else
            ...
            {
                here->BSIM4sNodePrime = ckt->add_internal_node("source");
            }
            else
            ...
            {
                here->BSIM4gNodePrime = ckt->add_internal_node("gate");
            }
            else
            ...
            {
                here->BSIM4gNodeMid = ckt->add_internal_node("midgate");
            }
            else
		...
		{
                    here->BSIM4dbNode = ckt->add_internal_node("dbody");
		}
		if (here->BSIM4bNodePrime == 0)
                {
                    here->BSIM4bNodePrime = ckt->add_internal_node("body");
                }
		if (here->BSIM4sbNode == 0)
                {
                    here->BSIM4sbNode = ckt->add_internal_node("sbody");
                }
            }
	    else
	    ...
	    {
                here->BSIM4qNode = ckt->add_internal_node("charge");
            }
	    else
	        here->BSIM4qNode = 0;
 ...
 * Z-port: SparsityBuilder append (no OOM path). Macro intentionally
 * expands to a brace-enclosed block (no trailing ';' required) so that
 * UCB's ~150 call sites — which have no trailing semicolon — remain
 * untouched. */
