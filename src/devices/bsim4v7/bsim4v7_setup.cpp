...
double Rtot, DMCGeff, DMCIeff, DMDGeff;
    ...
    {   /* process defaults of model parameters */
        ...
        if (!model->BSIM4v7versionGiven) 
            model->BSIM4v7version = strdup("4.7.0");
     ...
     if (!model->BSIM4v7pucsteGiven)
             ...
             here=here->BSIM4v7nextInstance) 
        {
            ...
            *states += BSIM4v7numStates;
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"drain");
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"source");
                if(error) return(error);
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"gate");
                if(error) return(error);
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"midgate");
                if(error) return(error);
            ...
            {   if (here->BSIM4v7dbNode == 0)
                {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"dbody");
                    if(error) return(error);
                    here->BSIM4v7dbNode = tmp->number;
                }
                if (here->BSIM4v7bNodePrime == 0)
                {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"body");
                    if(error) return(error);
                ...
                {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"sbody");
                    if(error) return(error);
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM4v7name,"charge");
                if(error) return(error);
            ...
            model = model->BSIM4v7nextModel)
    {
                ...
                here=here->BSIM4v7nextInstance)
        {
            if (here->BSIM4v7dNodePrime
                    && here->BSIM4v7dNodePrime != here->BSIM4v7dNode)
            {
                Shim::CKTdltNNum(ckt, here->BSIM4v7dNodePrime);
                here->BSIM4v7dNodePrime = 0;
            }
            if (here->BSIM4v7sNodePrime
                    && here->BSIM4v7sNodePrime != here->BSIM4v7sNode)
            {
                Shim::CKTdltNNum(ckt, here->BSIM4v7sNodePrime);
