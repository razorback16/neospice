      ...
      */
{
  ...
  for ( ;model != NULL ;model = model->HSM2nextModel ) {
	...
	*states += HSM2numStates;
	...
	error = Shim::CKTmkVolt(ckt, &tmp, here->HSM2name, "drain");
	...
	error = Shim::CKTmkVolt(ckt, &tmp, here->HSM2name, "source");
	if (error) return(error);
	...
	error = Shim::CKTmkVolt(ckt, &tmp, here->HSM2name, "gate");
	if (error) return(error);
	...
	if (here->HSM2dbNode == 0) {
	  error = Shim::CKTmkVolt(ckt, &tmp, here->HSM2name, "dbody");
	  if (error) return(error);
	  here->HSM2dbNode = tmp->number;
	}
	if (here->HSM2bNodePrime == 0) {
	  error = Shim::CKTmkVolt(ckt, &tmp,here->HSM2name, "body");
	  if (error) return(error);
	  here->HSM2bNodePrime = tmp->number;
	}
	if (here->HSM2sbNode == 0) {
	  error = Shim::CKTmkVolt(ckt, &tmp, here->HSM2name,"sbody");
	  if (error) return(error);
