...
struct HSM2ModelCard {
    ...
    HSM2ModelCard(const HSM2ModelCard&)            = delete;
    HSM2ModelCard& operator=(const HSM2ModelCard&) = delete;
};

class HSM2Device : public Device {
public:
    struct Geom {
        double W = 1e-6;
        double L = 1e-7;
    ...
    static std::unique_ptr<HSM2Device> make(
    ...
    int32_t state_vars() const override { return 21; }
...
private:
    explicit HSM2Device(std::string name) : Device(std::move(name)) {}

    mutable hisim2::HSM2Instance inst_{};
    hisim2::HSM2Model* model_ = nullptr;
