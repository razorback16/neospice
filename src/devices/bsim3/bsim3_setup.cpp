...
/* Default value Processing for BSIM3 MOSFET Models */
        ...
        if (!model->BSIM3versionGiven)
            model->BSIM3version = strdup("3.3.0");
...
/*        else
            ...
            *states += BSIM3numStates;
               ...
               )
            {
              if(here->BSIM3dNodePrime == 0) {
                error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM3name,"drain");
               ...
               )
            {
              if(here->BSIM3sNodePrime == 0) {
                error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM3name,"source");
                if(error) return(error);
 ...
 /* internal charge node */
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM3name,"charge");
                if(error) return(error);
            ...
            model = model->BSIM3nextModel)
    {
                ...
                here=here->BSIM3nextInstance)
        {
            if (here->BSIM3dNodePrime
                    && here->BSIM3dNodePrime != here->BSIM3dNode)
            {
                Shim::CKTdltNNum(ckt, here->BSIM3dNodePrime);
                here->BSIM3dNodePrime = 0;
            }
            if (here->BSIM3sNodePrime
                    && here->BSIM3sNodePrime != here->BSIM3sNode)
            {
                Shim::CKTdltNNum(ckt, here->BSIM3sNodePrime);
