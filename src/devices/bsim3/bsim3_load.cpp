...
line900:
           ...
           *(ckt->CKTrhs + here->BSIM3qNode) += m * (cqcheq - cqdef);
           ...
           T1 = qdef * here->BSIM3gtau;
           mat.add(here->BSIM3DdPtr, m * here->BSIM3drainConductance);
           mat.add(here->BSIM3GgPtr, m * (gcggb - ggtg));
           mat.add(here->BSIM3SsPtr, m * here->BSIM3sourceConductance);
           mat.add(here->BSIM3BbPtr, m * (here->BSIM3gbd + here->BSIM3gbs
                                - gcbgb - gcbdb - gcbsb - here->BSIM3gbbs));
           mat.add(here->BSIM3DPdpPtr, m * (here->BSIM3drainConductance
                                  + here->BSIM3gds + here->BSIM3gbd
                                  + RevSum + gcddb + dxpart * ggtd
                                  + T1 * ddxpart_dVd + gbdpdp));
           mat.add(here->BSIM3SPspPtr, m * (here->BSIM3sourceConductance
                                  + here->BSIM3gds + here->BSIM3gbs
                                  + FwdSum + gcssb + sxpart * ggts
                                  + T1 * dsxpart_dVs + gbspsp));
           mat.add(here->BSIM3DdpPtr, -(m * here->BSIM3drainConductance));
           mat.add(here->BSIM3GbPtr, -(m * (gcggb + gcgdb + gcgsb + ggtb)));
           mat.add(here->BSIM3GdpPtr, m * (gcgdb - ggtd));
           mat.add(here->BSIM3GspPtr, m * (gcgsb - ggts));
           mat.add(here->BSIM3SspPtr, -(m * here->BSIM3sourceConductance));
           mat.add(here->BSIM3BgPtr, m * (gcbgb - here->BSIM3gbgs));
           mat.add(here->BSIM3BdpPtr, m * (gcbdb - here->BSIM3gbd + gbbdp));
           mat.add(here->BSIM3BspPtr, m * (gcbsb - here->BSIM3gbs + gbbsp));
           mat.add(here->BSIM3DPdPtr, -(m * here->BSIM3drainConductance));
           mat.add(here->BSIM3DPgPtr, m * (Gm + gcdgb + dxpart * ggtg
                                 + T1 * ddxpart_dVg + gbdpg));
           mat.add(here->BSIM3DPbPtr, -(m * (here->BSIM3gbd - Gmbs + gcdgb + gcddb
                                 + gcdsb - dxpart * ggtb
                                 - T1 * ddxpart_dVb - gbdpb)));
           mat.add(here->BSIM3DPspPtr, -(m * (here->BSIM3gds + FwdSum - gcdsb
                                  - dxpart * ggts - T1 * ddxpart_dVs - gbdpsp)));
           mat.add(here->BSIM3SPgPtr, m * (gcsgb - Gm + sxpart * ggtg
                                 + T1 * dsxpart_dVg + gbspg));
           mat.add(here->BSIM3SPsPtr, -(m * here->BSIM3sourceConductance));
           mat.add(here->BSIM3SPbPtr, -(m * (here->BSIM3gbs + Gmbs + gcsgb + gcsdb
                                 + gcssb - sxpart * ggtb
                                 - T1 * dsxpart_dVb - gbspb)));
           mat.add(here->BSIM3SPdpPtr, -(m * (here->BSIM3gds + RevSum - gcsdb
                                  - sxpart * ggtd - T1 * dsxpart_dVd - gbspdp)));

           if (here->BSIM3nqsMod)
           {   mat.add(here->BSIM3QqPtr, m * (gqdef + here->BSIM3gtau));

               mat.add(here->BSIM3DPqPtr, m * (dxpart * here->BSIM3gtau));
               mat.add(here->BSIM3SPqPtr, m * (sxpart * here->BSIM3gtau));
               mat.add(here->BSIM3GqPtr, -(m * here->BSIM3gtau));

               mat.add(here->BSIM3QgPtr, m * (ggtg - gcqgb));
               mat.add(here->BSIM3QdpPtr, m * (ggtd - gcqdb));
               mat.add(here->BSIM3QspPtr, m * (ggts - gcqsb));
               mat.add(here->BSIM3QbPtr, m * (ggtb - gcqbb));
           }
line1000:  ;
#ifndef USE_OMP
