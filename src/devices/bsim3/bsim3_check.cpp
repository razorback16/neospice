...
int Fatal_Flag = 0;
    ...
    pParam = here->pParam;
    fprintf(stderr, "BSIM3v3.3.0 Parameter Checking.\n");
    if ((strncmp(model->BSIM3version, "3.3.0", 5)) && (strncmp(model->BSIM3version, "3.30", 4)) && (strncmp(model->BSIM3version, "3.3", 3)))
    {  fprintf(stderr, "Warning: This model is BSIM3v3.3.0; you specified a wrong version number.\n");
    }
    ...
    {   fprintf(stderr, "Fatal: Lintnoi = %g is too large - Leff for noise is negative.\n",
                model->BSIM3lintnoi);
        Fatal_Flag = 1;
    }
...
/* Check Saturation parameters */
    ...
    {   fprintf(stderr, "Warning: Rds at current temperature = %g is negative. Set to zero.\n",
                pParam->BSIM3rds0);
        pParam->BSIM3rds0 = 0.0;
    }
