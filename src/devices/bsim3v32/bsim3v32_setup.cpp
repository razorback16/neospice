...
int
BSIM3v32setup (Shim::Matrix *matrix, BSIM3v32Model *inModel, Shim::Ckt *ckt,
            int *states)
{
...
/* Default value Processing for BSIM3v32 MOSFET Models */
        ...
        if (!model->BSIM3v32versionGiven)
                model->BSIM3v32version = strdup("3.2.4");
            ...
            *states += BSIM3v32numStates;
               ...
               )
            {
               if(here->BSIM3v32dNodePrime == 0) {
                 error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM3v32name,"drain");
               ...
               )
            {
               if(here->BSIM3v32sNodePrime == 0) {
                 error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM3v32name,"source");
                 if(error) return(error);
            ...
            {   if(here->BSIM3v32qNode == 0)
                {   error = Shim::CKTmkVolt(ckt,&tmp,here->BSIM3v32name,"charge");
                    if(error) return(error);
            ...
            model = model->BSIM3v32nextModel)
    {
                ...
                here=here->BSIM3v32nextInstance)
        {
            if (here->BSIM3v32dNodePrime
                    && here->BSIM3v32dNodePrime != here->BSIM3v32dNode)
            {
                Shim::CKTdltNNum(ckt, here->BSIM3v32dNodePrime);
                here->BSIM3v32dNodePrime = 0;
            }
            if (here->BSIM3v32sNodePrime
                    && here->BSIM3v32sNodePrime != here->BSIM3v32sNode)
            {
                Shim::CKTdltNNum(ckt, here->BSIM3v32sNodePrime);
