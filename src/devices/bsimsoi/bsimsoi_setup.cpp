...
/* v2.0 release */
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"drain");
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"source");
                if(error) return(error);
...
/* v3.2 */
            ...
            else
            {
               if (here->B4SOIpNode == -1) {  /* floating body case -- 4-node */
                  error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"Body");
                  if (error) return(error);
            ...
            if ((model->B4SOIshMod == 1) && (here->B4SOIrth0!=0))
            {
               if (here->B4SOItempNode == -1) {
                  error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"Temp");
                  if (error) return(error);
...
/* v3.1 added for RF */
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"gate");
                if(error) return(error);
            ...
            {   error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"midgate");
                if(error) return(error);
...
/* v4.0 */
            ...
            {   if (here->B4SOIdbNode == 0)
                {   error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"dbody");
                    if(error) return(error);
                    here->B4SOIdbNode = tmp->number;
                }
                if (here->B4SOIsbNode == 0)
                {   error = Shim::CKTmkVolt(ckt,&tmp,here->B4SOIname,"sbody");
                    if(error) return(error);
...
/* here for debugging purpose only */
            ...
            {
               /* The real Vbs value */
               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Vbs");
               if(error) return(error);
               here->B4SOIvbsNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Ids");
               if(error) return(error);
               here->B4SOIidsNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Ic");
               if(error) return(error);
               here->B4SOIicNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Ibs");
               if(error) return(error);
               here->B4SOIibsNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Ibd");
               if(error) return(error);
               here->B4SOIibdNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Iii");
               if(error) return(error);
               here->B4SOIiiiNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Ig");
               if(error) return(error);
               here->B4SOIigNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Gigg");
               if(error) return(error);
               here->B4SOIgiggNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Gigd");
               if(error) return(error);
               here->B4SOIgigdNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Gigb");
               if(error) return(error);
               here->B4SOIgigbNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Igidl");
               if(error) return(error);
               here->B4SOIigidlNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Itun");
               if(error) return(error);
               here->B4SOIitunNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Ibp");
               if(error) return(error);
               here->B4SOIibpNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Cbb");
               if(error) return(error);
               here->B4SOIcbbNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Cbd");
               if(error) return(error);
               here->B4SOIcbdNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Cbg");
               if(error) return(error);
               here->B4SOIcbgNode = tmp->number;


               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Qbf");
               if(error) return(error);
               here->B4SOIqbfNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Qjs");
               if(error) return(error);
               here->B4SOIqjsNode = tmp->number;

               error = Shim::CKTmkVolt(ckt, &tmp, here->B4SOIname, "Qjd");
               if(error) return(error);
            ...
            model = model->B4SOInextModel)
    {
                ...
                here=here->B4SOInextInstance)
        {
            if (here->B4SOIdNodePrime
                    && here->B4SOIdNodePrime != here->B4SOIdNode)
            {
                Shim::CKTdltNNum(ckt, here->B4SOIdNodePrime);
                here->B4SOIdNodePrime = 0;
            }
            if (here->B4SOIsNodePrime
                    && here->B4SOIsNodePrime != here->B4SOIsNode)
            {
                Shim::CKTdltNNum(ckt, here->B4SOIsNodePrime);
                here->B4SOIsNodePrime = 0;
            }
        }
    }
