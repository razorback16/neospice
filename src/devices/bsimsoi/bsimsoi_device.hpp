#pragma once
// Adapter bridging the neospice Device interface to the UCB B4SOI code.
...
struct B4SOIModelCard {
    bsimsoi::B4SOIModel ucb{};   // aggregate UCB model fields

    B4SOIModelCard() = default;
    ~B4SOIModelCard();
    B4SOIModelCard(B4SOIModelCard&&) noexcept = delete;

    // Non-copyable / non-movable: UCB model is threaded with raw
    // pointers; copying would alias them.
    B4SOIModelCard(const B4SOIModelCard&)            = delete;
    B4SOIModelCard& operator=(const B4SOIModelCard&) = delete;
};
...
public:
    ...
    static std::unique_ptr<B4SOIDevice> make(
        std::string name, int32_t n_d, int32_t n_g, int32_t n_s, int32_t n_e, int32_t n_p, int32_t n_b,
    ...
    int32_t state_vars() const override { return 38; }
...
private:
    explicit B4SOIDevice(std::string name) : Device(std::move(name)) {}

    mutable bsimsoi::B4SOIInstance inst_{};
    bsimsoi::B4SOIModel* model_ = nullptr;
