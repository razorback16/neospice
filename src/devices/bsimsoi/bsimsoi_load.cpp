...
finished: /* returning Values to Calling Routine */
            ...
            *(ckt->CKTstate0 + here->B4SOIqbd) = qjd; /* v4.0 */
            ...
            {   if (Check == 1)
                ...
                {   double tol = ckt->CKTreltol * MAX(fabs(cdhat), fabs(here->B4SOIcd))
                    + ckt->CKTabstol;
...
line900:
            ...
            if (here->B4SOIrgateMod == 1)
            {
                mat.add(here->B4SOIGEgePtr, m * geltd);
                mat.add(here->B4SOIGgePtr, -(m * geltd));
                mat.add(here->B4SOIGEgPtr, -(m * geltd));
            }
            else if (here->B4SOIrgateMod == 2)
            {
                mat.add(here->B4SOIGEgePtr, m * gcrg);
                mat.add(here->B4SOIGEgPtr, m * gcrgg);
                mat.add(here->B4SOIGEdpPtr, m * gcrgd);
                mat.add(here->B4SOIGEspPtr, m * gcrgs);
                mat.add(here->B4SOIGgePtr, -(m * gcrg));
                if (here->B4SOIsoiMod !=2) /* v3.2 */
                    mat.add(here->B4SOIGEbPtr, m * gcrgb);
            }
            else if (here->B4SOIrgateMod == 3)
            {
                mat.add(here->B4SOIGEgePtr, m * geltd);
                mat.add(here->B4SOIGEgmPtr, -(m * geltd));
                mat.add(here->B4SOIGMgePtr, -(m * geltd));
                mat.add(here->B4SOIGMgmPtr, m * (geltd + gcrg + gcgmgmb));

                mat.add(here->B4SOIGMdpPtr, m * (gcrgd + gcgmdb));
                mat.add(here->B4SOIGMgPtr, m * gcrgg);
                mat.add(here->B4SOIGMspPtr, m * (gcrgs + gcgmsb));
                mat.add(here->B4SOIGMePtr, m * gcgmeb);
                if (here->B4SOIsoiMod !=2) /* v3.2 */
                    mat.add(here->B4SOIGMbPtr, m * gcrgb);

                mat.add(here->B4SOIDPgmPtr, m * gcdgmb);
                mat.add(here->B4SOIGgmPtr, -(m * gcrg));
                mat.add(here->B4SOISPgmPtr, m * gcsgmb);
                mat.add(here->B4SOIEgmPtr, m * gcegmb);
            }
            /* v3.1 added for RF end*/
            ...
            {
                mat.add(here->B4SOIDPePtr, m * (Gme + gddpe));
                mat.add(here->B4SOISPePtr, m * (gsspe - Gme));

                if (here->B4SOIsoiMod != 2) /* v3.2 */
                {
                    mat.add(here->B4SOIGePtr, m * gige);
                    mat.add(here->B4SOIBePtr, -(m * gige));
                }
            }

            mat.add(here->B4SOIEdpPtr, m * gcedb);
            mat.add(here->B4SOIEspPtr, m * gcesb);
            mat.add(here->B4SOIDPePtr, m * gcdeb);
            mat.add(here->B4SOISPePtr, m * gcseb);
            mat.add(here->B4SOIEgPtr, m * gcegb);
            mat.add(here->B4SOIGePtr, m * gcgeb);

            /* v3.1 */
            if (here->B4SOIsoiMod != 2) /* v3.2 */
            {
                mat.add(here->B4SOIEbPtr, -(m * (gcegb + gcedb + gcesb + gceeb + gcegmb))); /* 3.2 bug fix */

                /* v3.1 changed GbPtr for RF */
                if ((here->B4SOIrgateMod == 0) || (here->B4SOIrgateMod == 1))
                    mat.add(here->B4SOIGbPtr, -(m * (-gigb + gcggb + gcgdb + gcgsb
                     + gcgeb - gIgtotb)));
                else /* v3.1 for rgateMod = 2 or 3 */
                    mat.add(here->B4SOIGbPtr, m * (gigb + gcgbb +gIgtotb - gcrgb));


                mat.add(here->B4SOIDPbPtr, -(m * (-gddpb - Gmbs - gcdbb + gdtotb
                 + gIdtotb ))); /* v4.0 */
                ...
                mat.add(here->B4SOISPbPtr, -(m * (-gsspb + Gmbs - gcsbb + gstotb
                 + Gmin + gIstotb))); /* v4.0 */
                ...
                /*                      (*(here->B4SOISPbPtr) -= (-gsspb + Gmbs + gcsgb + gcsdb
                                        ...
                                        + Gmin + gIstotb);
                                        */
                mat.add(here->B4SOIBePtr, m * (gbbe + gcbeb)); /* v3.0 */
                mat.add(here->B4SOIBgPtr, m * (-gigg + gcbgb + gbbg));
                mat.add(here->B4SOIBdpPtr, m * (-gigd + gcbdb + gbbdp ));

                mat.add(here->B4SOIBspPtr, m * (gcbsb + gbbsp - Gmin
                 - gigs));
                /*                    if (!here->B4SOIrbodyMod)
                */
                mat.add(here->B4SOIBbPtr, m * (-gigb + gbbb - gcbgb - gcbdb
                 - gcbsb - gcbeb + Gmin));
                /*                    else
                ...
                /* v4.0 */
                if (here->B4SOIrbodyMod) {
                    mat.add(here->B4SOIDPdbPtr, m * (-gcjdbdp - GGjdb));
                    mat.add(here->B4SOISPsbPtr, m * (-gcjsbsp - GGjsb));
                    mat.add(here->B4SOIDBdpPtr, m * (-gcjdbdp - GGjdb));
                    mat.add(here->B4SOIDBdbPtr, m * (gcjdbdp + GGjdb
                     + here->B4SOIgrbdb));
                    mat.add(here->B4SOIDBbPtr, -(m * here->B4SOIgrbdb));
                    mat.add(here->B4SOISBspPtr, m * (-gcjsbsp - GGjsb));
                    mat.add(here->B4SOISBbPtr, -(m * here->B4SOIgrbsb));
                    mat.add(here->B4SOISBsbPtr, m * (gcjsbsp + GGjsb
                     + here->B4SOIgrbsb));
                    mat.add(here->B4SOIBdbPtr, -(m * here->B4SOIgrbdb));
                    mat.add(here->B4SOIBsbPtr, -(m * here->B4SOIgrbsb));
                    mat.add(here->B4SOIBbPtr, m * (here->B4SOIgrbsb
                     + here->B4SOIgrbdb));
                }
                if (model->B4SOIrdsMod)
                {
                    mat.add(here->B4SOIDbPtr, m * gdtotb);
                    mat.add(here->B4SOISbPtr, m * gstotb);
                }

            }
            /* v3.1 */
            if (model->B4SOIrdsMod)
            {   mat.add(here->B4SOIDgPtr, m * gdtotg);
                mat.add(here->B4SOIDspPtr, m * gdtots);
                mat.add(here->B4SOISdpPtr, m * gstotd);
                mat.add(here->B4SOISgPtr, m * gstotg);
            }

            mat.add(here->B4SOIEePtr, m * gceeb);

            if (here->B4SOIrgateMod == 0)
            {
                mat.add(here->B4SOIGgPtr, m * (gigg + gcggb + Gmin
                 + gIgtotg));
                mat.add(here->B4SOIGdpPtr, m * (gigd + gcgdb - Gmin
                 + gIgtotd));
                mat.add(here->B4SOIGspPtr, m * (gcgsb + gigs + gIgtots));
            }
            ...
            else /* v3.1 for RF rgateMod == 2 or 3 */
            {
                ...
                mat.add(here->B4SOIGdpPtr, m * (gigd + gcgdb - Gmin
                    + gIgtotd - gcrgd));
                mat.add(here->B4SOIGspPtr, m * (gcgsb + gigs + gIgtots - gcrgs));
            }


            mat.add(here->B4SOIDPgPtr, m * ((Gm + gcdgb) + gddpg - Gmin
             - gIdtotg - gdtotg)); /* v4.0 */
            mat.add(here->B4SOIDPdpPtr, m * ((gdpr + here->B4SOIgds + gddpdp
                                       + RevSum + gcddb) + Gmin
             - gIdtotd - gdtotd)); /* v4.0 */
            mat.add(here->B4SOIDPspPtr, -(m * ((-gddpsp + here->B4SOIgds + FwdSum
                                       - gcdsb) + gIdtots + gdtots)));

            mat.add(here->B4SOIDPdPtr, -(m * (gdpr + gdtot)));

            mat.add(here->B4SOISPgPtr, m * (gcsgb - Gm + gsspg - gIstotg
             - gstotg)); /* v4.0 */
            mat.add(here->B4SOISPdpPtr, -(m * ((here->B4SOIgds - gsspdp + RevSum
                                       - gcsdb + gIstotd) + gstotd))); /* v4.0 */

            mat.add(here->B4SOISPspPtr, m * ((gspr - gstots
                                       + here->B4SOIgds + gsspsp
                                       + FwdSum + gcssb)
             + Gmin - gIstots)); /* v4.0 */

            mat.add(here->B4SOISPsPtr, -(m * (gspr + gstot)));


            mat.add(here->B4SOIDdPtr, m * (gdpr + gdtot));
            mat.add(here->B4SOIDdpPtr, -(m * (gdpr - gdtotd)));


            mat.add(here->B4SOISsPtr, m * (gspr + gstot));
            mat.add(here->B4SOISspPtr, -(m * (gspr - gstots)));


            if (here->B4SOIbodyMod == 1)  {
                mat.add(here->B4SOIBpPtr, -(m * gppp));
                mat.add(here->B4SOIPbPtr, m * gppb);
                mat.add(here->B4SOIPpPtr, m * gppp);
            }

            /* v4.1  Ig_agbcp2 stamping */
            mat.add(here->B4SOIGgPtr, m * gigpg);
            if (here->B4SOIbodyMod == 1)  {
                mat.add(here->B4SOIPpPtr, -(m * gigpp));
                mat.add(here->B4SOIPgPtr, -(m * gigpg));
                mat.add(here->B4SOIGpPtr, m * gigpp);
            }
            else if(here->B4SOIbodyMod == 2)
            {
                mat.add(here->B4SOIBbPtr, -(m * gigpp));
                mat.add(here->B4SOIBgPtr, -(m * gigpg));
                mat.add(here->B4SOIGbPtr, m * gigpp);
            }


            if (selfheat)
            {
                mat.add(here->B4SOIDPtempPtr, m * (GmT + gddpT + gcdT));
                mat.add(here->B4SOISPtempPtr, m * (-GmT + gsspT + gcsT));
                mat.add(here->B4SOIBtempPtr, m * (gbbT + gcbT - gigT));
                mat.add(here->B4SOIEtempPtr, m * gceT);
                mat.add(here->B4SOIGtempPtr, m * (gcgT + gigT));
                mat.add(here->B4SOITemptempPtr, m * (gTtt  + 1/pParam->B4SOIrth + gcTt));
                mat.add(here->B4SOITempgPtr, m * gTtg);
                mat.add(here->B4SOITempbPtr, m * gTtb);
                mat.add(here->B4SOITempdpPtr, m * gTtdp);
                mat.add(here->B4SOITempspPtr, m * gTtsp);

                /* v3.0 */
                if (here->B4SOIsoiMod != 0) /* v3.2 */
                    mat.add(here->B4SOITempePtr, m * gTte);
            ...
            if (here->B4SOIdebugMod != 0)
            {
                mat.add(here->B4SOIVbsPtr, 1);
                mat.add(here->B4SOIIdsPtr, 1);
                mat.add(here->B4SOIIcPtr, 1);
                mat.add(here->B4SOIIbsPtr, 1);
                mat.add(here->B4SOIIbdPtr, 1);
                mat.add(here->B4SOIIiiPtr, 1);
                mat.add(here->B4SOIIgPtr, 1);
                mat.add(here->B4SOIGiggPtr, 1);
                mat.add(here->B4SOIGigdPtr, 1);
                mat.add(here->B4SOIGigbPtr, 1);
                mat.add(here->B4SOIIgidlPtr, 1);
                mat.add(here->B4SOIItunPtr, 1);
                mat.add(here->B4SOIIbpPtr, 1);
                mat.add(here->B4SOICbgPtr, 1);
                mat.add(here->B4SOICbbPtr, 1);
                mat.add(here->B4SOICbdPtr, 1);
                mat.add(here->B4SOIQbfPtr, 1);
                mat.add(here->B4SOIQjsPtr, 1);
                mat.add(here->B4SOIQjdPtr, 1);
            }
