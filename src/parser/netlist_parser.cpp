...
Circuit NetlistParser::parse(const std::string& netlist) {
    ...
    struct DeferredMosfet {
        ...
        int32_t npnode = GROUND_INTERNAL;  // optional 6th terminal (body/p-node) for BSIMSOI
        int32_t nbulk = GROUND_INTERNAL;   // optional 7th (bulk) for BSIMSOI
    ...
    // Deferred HFETs: parsed Z-cards with nhfet/phfet model types resolved
    // in a second pass once all .model cards are known. LEVEL determines device:
    ...
    std::vector<DeferredHFET> deferred_hfets;
    ...
    for (const auto& line : lines) {
        ...
        if (elem_type == 'r') {
            ...
            if (!tok5_is_model && tokens.size() >= 7 &&
                ...
                bool tok6_is_model = (models.find(tokens[6]) != models.end() ||
                                      models.find(tok6_lower) != models.end());
    ...
    for (const auto& m : deferred_mosfets) {
        ...
        if (level == 1) {
            ...
            // BSIMSOI terminal mapping from DeferredMosfet:
