...
def generate_shim_hpp(desc: _Desc) -> str:
    ...
    parts.append("#endif\n")
    parts.append("#ifndef OFF\n")
    parts.append("#define OFF 0\n")
    parts.append("#endif\n")
    parts.append("\n")
    parts.append("// UCB physical constants (used in temp and load functions).\n")
    parts.append("#ifndef CONSTKoverQ\n")
    parts.append("#define CONSTKoverQ 8.6171e-05       // Boltzmann/charge  (eV/K)\n")
    parts.append("#endif\n")
    parts.append("#ifndef CONSTe\n")
    parts.append("#define CONSTe 2.7182818284590452354 // Euler's number\n")
    parts.append("#endif\n")
    parts.append("#ifndef CONSTboltz\n")
    parts.append("#define CONSTboltz 1.3806226e-23\n")
    parts.append("#endif\n")
    parts.append("#ifndef REFTEMP\n")
    parts.append("#define REFTEMP 300.15\n")
    parts.append("#endif\n")
    parts.append("\n")
    ...
    parts.append("    constexpr int IF_COMPLEX   = 0x20;\n")
    parts.append("\n")
    ...
    parts.append("\n")
    parts.append("        void *CKTtroubleElt = nullptr;\n")
    parts.append("        void *CKTsenInfo = nullptr;\n")
    parts.append("\n")
    parts.append("        int CKTinternalNodeCounter = 1000;\n")
    parts.append("        int CKTcopyNodesets = 0;\n")
    parts.append("\n")
    parts.append("        // Stub for sensitivity gating (CKTcurJob->JOBtype check)\n")
    parts.append("        struct { int JOBtype = 0; } *CKTcurJob = nullptr;\n")
    parts.append("\n")
    ...
    parts.append("\n")
    parts.append("    // --- UCB CKTdltNNum stub (no-op: neospice owns node cleanup) ----------\n")
    parts.append("    inline void CKTdltNNum(Ckt * /*ckt*/, int /*node*/) {}\n")
    parts.append("\n")
    parts.append("    // --- UCB CKTnode + CKTmkVolt replacement --------------------------------\n")
    parts.append("    struct CKTnode {\n")
    parts.append("        int number = 0;\n")
    parts.append("        const char *name = nullptr;\n")
    parts.append("        double *nodeset = nullptr;\n")
    parts.append("        double *ic = nullptr;\n")
    parts.append("        int nsGiven = 0;\n")
    parts.append("        int icGiven = 0;\n")
    parts.append("    };\n")
    parts.append("    int CKTmkVolt(Ckt *ckt, CKTnode **node_out,\n")
    parts.append("                  const char *basename, const char *suffix);\n")
    parts.append("    inline int CKTinst2Node(Ckt * /*ckt*/, void * /*inst*/, int /*term*/,\n")
    parts.append("                            CKTnode ** /*node*/, const char ** /*name*/) {\n")
    parts.append("        return E_UNSUPP;\n")
    parts.append("    }\n")
    ...
    # --- Close Shim namespace ------------------------------------------------
    parts.append("} // namespace Shim\n")
    parts.append("\n")

    # --- Device support functions (DEVfetlim etc.) -------------------------
    # These are implemented in the translated *_devsup.cpp and live in the
    # outer model namespace, not inside Shim.
    parts.append("// --- UCB device limiting functions (devsup.c) ---\n")
    parts.append("double DEVlimvds  (double vnew, double vold);\n")
    parts.append("double DEVpnjlim  (double vnew, double vold, double vt, double vcrit, int *icheck);\n")
    parts.append("double DEVfetlim  (double vnew, double vold, double vto);\n")
    parts.append("\n")
...
def generate_shim_cpp(desc: _Desc) -> str:
    ...
    parts.append("int CKTmkVolt(Ckt *ckt, CKTnode **node_out,\n")
    parts.append("              const char * /*basename*/, const char *suffix) {\n")
    parts.append("    int idx = ckt->add_internal_node(suffix);\n")
    parts.append("    s_tmp_node_.number = idx;\n")
    parts.append("    s_tmp_node_.name   = suffix;\n")
    parts.append("    if (node_out) *node_out = &s_tmp_node_;\n")
    ...
    # --- UCB device limiting functions (from ngspice devsup.c) ---------------
    parts.append(f"namespace neospice::{ns} {{\n")
    parts.append("\n")
    parts.append("double DEVlimvds(double vnew, double vold) {\n")
    parts.append("    if(vold >= 3.5) {\n")
    parts.append("        if(vnew > vold) {\n")
    parts.append("            vnew = std::fmin(vnew,(3 * vold) +2);\n")
    parts.append("        } else {\n")
    parts.append("            if (vnew < 3.5) vnew = std::fmax(vnew,2);\n")
    parts.append("        }\n")
    parts.append("    } else {\n")
    parts.append("        if(vnew > vold) vnew = std::fmin(vnew,4);\n")
    parts.append("        else vnew = std::fmax(vnew,-.5);\n")
    parts.append("    }\n")
    parts.append("    return vnew;\n")
    parts.append("}\n")
    parts.append("\n")
    parts.append("double DEVpnjlim(double vnew, double vold, double vt, double vcrit, int *icheck) {\n")
    parts.append("    double arg;\n")
    parts.append("    if((vnew > vcrit) && (std::fabs(vnew - vold) > (vt + vt))) {\n")
    parts.append("        if(vold > 0) {\n")
    parts.append("            arg = (vnew - vold) / vt;\n")
    parts.append("            if(arg > 0) vnew = vold + vt * (2+std::log(arg-2));\n")
    parts.append("            else vnew = vold - vt * (2+std::log(2-arg));\n")
    parts.append("        } else {\n")
    parts.append("            vnew = vt *std::log(vnew/vt);\n")
    parts.append("        }\n")
    parts.append("        *icheck = 1;\n")
    parts.append("    } else {\n")
    parts.append("       if (vnew < 0) {\n")
    parts.append("           if (vold > 0) arg = -1*vold-1;\n")
    parts.append("           else arg = 2*vold-1;\n")
    parts.append("           if (vnew < arg) { vnew = arg; *icheck = 1; }\n")
    parts.append("           else *icheck = 0;\n")
    parts.append("        } else *icheck = 0;\n")
    parts.append("    }\n")
    parts.append("    return vnew;\n")
    parts.append("}\n")
    parts.append("\n")
    parts.append("double DEVfetlim(double vnew, double vold, double vto) {\n")
    parts.append("    double vtsthi = std::fabs(2*(vold-vto))+2;\n")
    parts.append("    double vtstlo = std::fabs(vold-vto)+1;\n")
    parts.append("    double vtox   = vto + 3.5;\n")
    parts.append("    double delv   = vnew-vold;\n")
    parts.append("    double vtemp;\n")
    parts.append("    if (vold >= vto) {\n")
    parts.append("        if(vold >= vtox) {\n")
    parts.append("            if(delv <= 0) {\n")
    parts.append("                if(vnew >= vtox) {\n")
    parts.append("                    if(-delv >vtstlo) vnew = vold - vtstlo;\n")
    parts.append("                } else vnew = std::fmax(vnew,vto+2);\n")
    parts.append("            } else {\n")
    parts.append("                if(delv >= vtsthi) vnew = vold + vtsthi;\n")
    parts.append("            }\n")
    parts.append("        } else {\n")
    parts.append("            if(delv <= 0) vnew = std::fmax(vnew,vto-.5);\n")
    parts.append("            else vnew = std::fmin(vnew,vto+4);\n")
    parts.append("        }\n")
    parts.append("    } else {\n")
    parts.append("        if(delv <= 0) {\n")
    parts.append("            if(-delv >vtsthi) vnew = vold - vtsthi;\n")
    parts.append("        } else {\n")
    parts.append("            vtemp = vto + .5;\n")
    parts.append("            if(vnew <= vtemp) {\n")
    parts.append("                if(delv >vtstlo) vnew = vold + vtstlo;\n")
    parts.append("            } else vnew = vtemp;\n")
    parts.append("        }\n")
    parts.append("    }\n")
    parts.append("    return vnew;\n")
    parts.append("}\n")
